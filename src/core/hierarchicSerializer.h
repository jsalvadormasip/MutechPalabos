...
public:
    HierarchicUnserializer(std::vector<char> const &data_, pluint currentPos_) :
        data(data_), currentPos(currentPos_), idIndirect(0)
    {
        readHeader();
    }
    HierarchicUnserializer(
        std::vector<char> const &data_, pluint currentPos_, std::map<int, int> const *idIndirect_) :
        data(data_), currentPos(currentPos_), idIndirect(idIndirect_)
    {
        readHeader();
    }
    ...
    template <typename T>
    bool readValues(std::vector<T> &values)
    {
    ...
    bool readValues(Array<T, ndim> &values)
    {
...
public:
    HierarchicSerializer(std::vector<char> &data_, int topMostObjectId) :
        data(data_), currentPos(data.size())
    {
    ...
    template <typename T>
    void addValues(std::vector<T> const &values)
    {
        PLB_PRECONDITION(currentPos > sizeof(int));
        if (values.empty())
            return;
    ...
    template <typename T, int ndim>
    void addValues(Array<T, ndim> const &values)
    {
