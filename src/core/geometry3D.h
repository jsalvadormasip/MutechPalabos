...
struct Box3D {
    ...
    {
        return Box3D(
            scaling * x0, scaling * x1, scaling * y0, scaling * y1, scaling * z0, scaling * z1);
    }
    /// Return same box, rescaled by a factor 1/scaling
    Box3D divide(plint scaling) const
    {
        return Box3D(
            x0 / scaling, x1 / scaling, y0 / scaling, y1 / scaling, z0 / scaling, z1 / scaling);
    }
    ...
    {
        return Box3D(
            util::roundUp(x0, scaling) / scaling, util::roundDown(x1, scaling) / scaling,
    ...
    {
        return Box3D(
            util::roundDown(x0, scaling) / scaling, util::roundUp(x1, scaling) / scaling,
    ...
    {
        return Box3D(
            x0 + array[0], x1 + array[1], y0 + array[2], y1 + array[3], z0 + array[4],
            z1 + array[5]);
    }
    ...
    /// Add a border of nCells cells to the box in the two directions normal to dir (x=0, y=1, z=2)
    Box3D enlargeInNormalPlane(
        const Array<plint, 2> &nCellsOne, const Array<plint, 2> &nCellsTwo, plint dir) const
    {
        switch (dir) {
        case 0:
            return Box3D(
                x0, x1, y0 - nCellsOne[0], y1 + nCellsOne[1], z0 - nCellsTwo[0], z1 + nCellsTwo[1]);
        case 1:
            return Box3D(
                x0 - nCellsTwo[0], x1 + nCellsTwo[1], y0, y1, z0 - nCellsOne[0], z1 + nCellsOne[1]);
        case 2:
            return Box3D(
                x0 - nCellsOne[0], x1 + nCellsOne[1], y0 - nCellsTwo[0], y1 + nCellsTwo[1], z0, z1);
        default:
            PLB_ASSERT(false && "The direction must be 0, 1, or 2");
            break;
        }
    ...
    {
        return util::val2str(x0) + ", " + util::val2str(x1) + ", " + util::val2str(y0) + ", "
               + util::val2str(y1) + ", " + util::val2str(z0) + ", " + util::val2str(z1);
    }

    bool operator==(Box3D const &rhs) const
    {
        return x0 == rhs.x0 && y0 == rhs.y0 && z0 == rhs.z0 && x1 == rhs.x1 && y1 == rhs.y1
               && z1 == rhs.z1;
    }
...
inline bool operator<(Dot3D const &dot1, Dot3D const &dot2)
{
    return (dot1.x < dot2.x)
           || ((dot1.x == dot2.x)
               && ((dot1.y < dot2.y) || ((dot1.y == dot2.y) && (dot1.z < dot2.z))));
}
...
{
    return x[0] > box.x0 && x[0] < box.x1 && x[1] > box.y0 && x[1] < box.y1 && x[2] > box.z0
           && x[2] < box.z1;
}
...
{
    return x[0] - epsilon > box.x0 && x[0] + epsilon < box.x1 && x[1] - epsilon > box.y0
           && x[1] + epsilon < box.y1 && x[2] - epsilon > box.z0 && x[2] + epsilon < box.z1;
}

/// Decide if a Lagrangian point is contained in 3D box, boundaries inclusive at the epsilon level
/// of accuracy.
template <typename T>
inline bool containedInclusive(Array<T, 3> const &x, Box3D const &box, T epsilon)
{
    return util::greaterEqual_abs(x[0], (T)box.x0, epsilon)
           && util::lessEqual_abs(x[0], (T)box.x1, epsilon)
           && util::greaterEqual_abs(x[1], (T)box.y0, epsilon)
           && util::lessEqual_abs(x[1], (T)box.y1, epsilon)
           && util::greaterEqual_abs(x[2], (T)box.z0, epsilon)
           && util::lessEqual_abs(x[2], (T)box.z1, epsilon);
}

/// Decide if a Lagrangian point is contained in 3D box, boundaries inclusive at the floating point
...
{
    return box1.x0 >= box2.x0 && box1.x1 <= box2.x1 && box1.y0 >= box2.y0 && box1.y1 <= box2.y1
...
{
    if (box1.x0 == box2.x0 && box1.x1 == box2.x1 && box1.y0 == box2.y0 && box1.y1 == box2.y1) {
    ...
    if (box1.x0 == box2.x0 && box1.x1 == box2.x1 && box1.z0 == box2.z0 && box1.z1 == box2.z1) {
    ...
    if (box1.y0 == box2.y0 && box1.y1 == box2.y1 && box1.z0 == box2.z0 && box1.z1 == box2.z1) {
...
inline void except(Box3D const &originalBox, Box3D const &toExcept, std::vector<Box3D> &result)
{
    ...
    if (!doesIntersect) {
        ...
        if (intersection.x0 != originalBox.x0) {
            result.push_back(Box3D(
                originalBox.x0, intersection.x0 - 1, originalBox.y0, originalBox.y1, originalBox.z0,
                originalBox.z1));
        }
        if (intersection.x1 != originalBox.x1) {
            result.push_back(Box3D(
                intersection.x1 + 1, originalBox.x1, originalBox.y0, originalBox.y1, originalBox.z0,
                originalBox.z1));
        }
        if (intersection.y0 != originalBox.y0) {
            result.push_back(Box3D(
                intersection.x0, intersection.x1, originalBox.y0, intersection.y0 - 1,
                originalBox.z0, originalBox.z1));
        }
        if (intersection.y1 != originalBox.y1) {
            result.push_back(Box3D(
                intersection.x0, intersection.x1, intersection.y1 + 1, originalBox.y1,
                originalBox.z0, originalBox.z1));
        }
        if (intersection.z0 != originalBox.z0) {
            result.push_back(Box3D(
                intersection.x0, intersection.x1, intersection.y0, intersection.y1, originalBox.z0,
                intersection.z0 - 1));
        }
        if (intersection.z1 != originalBox.z1) {
            result.push_back(Box3D(
                intersection.x0, intersection.x1, intersection.y0, intersection.y1,
                intersection.z1 + 1, originalBox.z1));
        }
    }
}
...
{
    return Box3D(
        std::min(box1.x0, box2.x0), std::max(box1.x1, box2.x1), std::min(box1.y0, box2.y0),
        std::max(box1.y1, box2.y1), std::min(box1.z0, box2.z0), std::max(box1.z1, box2.z1));
}
...
template <typename T>
struct Plane {
    Plane(Array<T, 3> point_, Array<T, 3> normal_) : point(point_), normal(normal_) { }
    Plane()
    {
        point.resetToZero();
        normal.resetToZero();
...
struct Cuboid {
    ...
    Cuboid(Array<T, 3> const &lowerLeftCorner_, Array<T, 3> const &upperRightCorner_) :
        lowerLeftCorner(lowerLeftCorner_), upperRightCorner(upperRightCorner_)
    { }
...
{
    return Box3D(
        (plint)c.x0(), (plint)c.x1() + 1, (plint)c.y0(), (plint)c.y1() + 1, (plint)c.z0(),
        (plint)c.z1() + 1);
}
...
inline bool contained(Array<T, 3> const &x, Cuboid<T> const &c)
{
    ...
    return x[0] >= llc[0] && x[0] <= urc[0] && x[1] >= llc[1] && x[1] <= urc[1] && x[2] >= llc[2]
           && x[2] <= urc[2];
}
...
inline bool contained(
    Cuboid<T> const &c1, Cuboid<T> const &c2, T eps = std::numeric_limits<T>::epsilon())
{
    return util::greaterThan(c1.x0(), c2.x0(), eps) && util::lessThan(c1.x0(), c2.x1(), eps)
           && util::greaterThan(c1.x1(), c2.x0(), eps) && util::lessThan(c1.x1(), c2.x1(), eps)
           && util::greaterThan(c1.y0(), c2.y0(), eps) && util::lessThan(c1.y0(), c2.y1(), eps)
           && util::greaterThan(c1.y1(), c2.y0(), eps) && util::lessThan(c1.y1(), c2.y1(), eps)
           && util::greaterThan(c1.z0(), c2.z0(), eps) && util::lessThan(c1.z0(), c2.z1(), eps)
           && util::greaterThan(c1.z1(), c2.z0(), eps) && util::lessThan(c1.z1(), c2.z1(), eps);
}

/// Decide if a cuboid is contained in another cuboid, boundaries inclusive.
template <typename T>
inline bool containedInclusive(
    Cuboid<T> const &c1, Cuboid<T> const &c2, T eps = std::numeric_limits<T>::epsilon())
{
    return util::greaterEqual(c1.x0(), c2.x0(), eps) && util::lessEqual(c1.x0(), c2.x1(), eps)
           && util::greaterEqual(c1.x1(), c2.x0(), eps) && util::lessEqual(c1.x1(), c2.x1(), eps)
           && util::greaterEqual(c1.y0(), c2.y0(), eps) && util::lessEqual(c1.y0(), c2.y1(), eps)
           && util::greaterEqual(c1.y1(), c2.y0(), eps) && util::lessEqual(c1.y1(), c2.y1(), eps)
           && util::greaterEqual(c1.z0(), c2.z0(), eps) && util::lessEqual(c1.z0(), c2.z1(), eps)
           && util::greaterEqual(c1.z1(), c2.z0(), eps) && util::lessEqual(c1.z1(), c2.z1(), eps);
}
...
inline int lineIntersectionWithPlane(
    Plane<T> const &plane, Array<T, 3> const &point1, Array<T, 3> const &point2, T epsilon,
...
public:
    PositionLessThan3D(T epsilon_) : epsilon(epsilon_) { }
...
private:
    ...
    {
        return (
            (positionCoordinateLessThan(p1[0], p2[0])
