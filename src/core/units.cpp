...
Units3D::Units3D(
    Array<double, 3> const &lowerLeftCorner, Array<double, 3> const &upperRightCorner) :
    lbDomain_(0, 0, 0, 0, 0, 0),
...
Array<double, 3> Units3D::lbForce(Array<double, 3> const &physForce, double rho0) const
{
    return Array<double, 3>(
        lbForce(physForce[0], rho0), lbForce(physForce[1], rho0), lbForce(physForce[2], rho0));
}

Array<double, 3> Units3D::physForce(Array<double, 3> const &lbForce, double rho0) const
{
    return Array<double, 3>(
        physForce(lbForce[0], rho0), physForce(lbForce[1], rho0), physForce(lbForce[2], rho0));
}
...
{
    return Array<double, 3>(
        lbTorque(physTorque[0], rho0), lbTorque(physTorque[1], rho0),
...
{
    return Array<double, 3>(
        physTorque(lbTorque[0], rho0), physTorque(lbTorque[1], rho0),
...
{
    dx_      = dx__;
    plint nx = adjustLength(physDomain_.lowerLeftCorner[0], physDomain_.upperRightCorner[0]);
    plint ny = adjustLength(physDomain_.lowerLeftCorner[1], physDomain_.upperRightCorner[1]);
    plint nz = adjustLength(physDomain_.lowerLeftCorner[2], physDomain_.upperRightCorner[2]);
    if (periodicX_ && nx > 0) {
        --nx;
    }
    if (periodicY_ && ny > 0) {
...
void Units3D::setResolution(plint n, plint direction)
{
    ...
    PLB_ASSERT(n > 0);
    double l = physDomain_.upperRightCorner[direction] - physDomain_.lowerLeftCorner[direction];
    set_dx(l / n);
}
...
void Units3D::extend(plint dx0, plint dx1, plint dy0, plint dy1, plint dz0, plint dz1)
{
    ...
    PLB_ASSERT(
        physDomain_.lowerLeftCorner[0] - dx0 * dx_ + dx_
        < physDomain_.upperRightCorner[0] + dx1 * dx_);
    PLB_ASSERT(
        physDomain_.lowerLeftCorner[1] - dy0 * dx_ + dx_
        < physDomain_.upperRightCorner[1] + dy1 * dx_);
    PLB_ASSERT(
        physDomain_.lowerLeftCorner[2] - dz0 * dx_ + dx_
    ...
    lbDomain_ = Box3D(
        0, lbDomain_.getNx() - 1 + dx0 + dx1, 0, lbDomain_.getNy() - 1 + dy0 + dy1, 0,
        lbDomain_.getNz() - 1 + dz0 + dz1);
}
...
void Units3D::extend1()
{
    ...
    lbDomain_ = Box3D(0, lbDomain_.getNx(), 0, lbDomain_.getNy(), 0, lbDomain_.getNz());
}
