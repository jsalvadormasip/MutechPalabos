...
void BoundedBoxScalarSumFunctional2D<T>::processEdge(
    [[maybe_unused]] int direction, [[maybe_unused]] int orientation, Box2D domain, ScalarField2D<T> &scalarField)
{
    BlockStatistics &statistics = this->getStatistics();
    for (plint iX = domain.x0; iX <= domain.x1; ++iX) {
        for (plint iY = domain.y0; iY <= domain.y1; ++iY) {
...
void BoundedBoxScalarSumFunctional2D<T>::processCorner(
    [[maybe_unused]] int normalX, [[maybe_unused]] int normalY, Box2D domain, ScalarField2D<T> &scalarField)
{
    BlockStatistics &statistics = this->getStatistics();
    for (plint iX = domain.x0; iX <= domain.x1; ++iX) {
        for (plint iY = domain.y0; iY <= domain.y1; ++iY) {
