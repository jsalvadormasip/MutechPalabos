...
void BoundedBoxScalarSumFunctional3D<T>::processPlane(
    [[maybe_unused]] int direction, [[maybe_unused]] int orientation, Box3D domain, ScalarField3D<T> &scalarField)
{
    BlockStatistics &statistics = this->getStatistics();
    for (plint iX = domain.x0; iX <= domain.x1; ++iX) {
        for (plint iY = domain.y0; iY <= domain.y1; ++iY) {
...
void BoundedBoxScalarSumFunctional3D<T>::processEdge(
    [[maybe_unused]] int plane, [[maybe_unused]] int normal1, [[maybe_unused]] int normal2, Box3D domain, ScalarField3D<T> &scalarField)
{
    BlockStatistics &statistics = this->getStatistics();
    for (plint iX = domain.x0; iX <= domain.x1; ++iX) {
        for (plint iY = domain.y0; iY <= domain.y1; ++iY) {
...
void BoundedBoxScalarSumFunctional3D<T>::processCorner(
    [[maybe_unused]] int normalX, [[maybe_unused]] int normalY, [[maybe_unused]] int normalZ, Box3D domain, ScalarField3D<T> &scalarField)
{
    BlockStatistics &statistics = this->getStatistics();
    for (plint iX = domain.x0; iX <= domain.x1; ++iX) {
        for (plint iY = domain.y0; iY <= domain.y1; ++iY) {
