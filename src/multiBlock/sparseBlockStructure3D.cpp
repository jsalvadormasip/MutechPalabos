    ...
    boundingBox(boundingBox_)
{
    PLB_PRECONDITION(
        boundingBox.getNx() >= 1 && boundingBox.getNy() >= 1 && boundingBox.getNz() >= 1);
    gridNx = (plint)(
        0.5 + (double)rhs.gridNx * (double)boundingBox.getNx() / (double)rhs.boundingBox.getNx());
    if (gridNx < 1)
        gridNx = 1;
    gridNy = (plint)(
        0.5 + (double)rhs.gridNy * (double)boundingBox.getNy() / (double)rhs.boundingBox.getNy());
    if (gridNy < 1)
        gridNy = 1;
    gridNz = (plint)(
        0.5 + (double)rhs.gridNz * (double)boundingBox.getNz() / (double)rhs.boundingBox.getNz());
    if (gridNz < 1)
        gridNz = 1;
}
