...
RandomRedistribute3D::RandomRedistribute3D(pluint rseed_) : rseed(rseed_) { }
    ...
    MultiBlockManagement3D const &original) const
{
    ...
    return MultiBlockManagement3D(
        originalSparseBlock, newAttribution, original.getEnvelopeWidth(),
        original.getRefinementLevel());
}
