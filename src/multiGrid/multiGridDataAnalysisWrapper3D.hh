...
void computeVorticity(
    MultiGridTensorField3D<T, 2> &velocity, MultiGridScalarField3D<T> &vorticity, Box3D domain)
{
    plint envelopeWidth = 1;
    applyProcessingFunctional(
        new BoxVorticityFunctional3D<T, 2>, domain, vorticity, velocity,
        velocity.getReferenceLevel(), envelopeWidth);
}

template <typename T>
std::unique_ptr<MultiGridScalarField3D<T> > computeVorticity(
    MultiGridTensorField3D<T, 2> &velocity, Box3D domain)
{
...
{
    applyProcessingFunctional(
        new BoxBulkVorticityFunctional3D<T, 2>, domain, vorticity, velocity,
        velocity.getReferenceLevel());
}

template <typename T>
std::unique_ptr<MultiGridScalarField3D<T> > computeBulkVorticity(
    MultiGridTensorField3D<T, 2> &velocity, Box3D domain)
{
    MultiGridScalarField3D<T> *vorticity = new MultiGridScalarField3D<T>(velocity, domain);
...
template <typename T>
std::unique_ptr<MultiGridScalarField3D<T> > computeBulkVorticity(
    MultiGridTensorField3D<T, 2> &velocity)
{
...
void computeStrainRate(
    MultiGridTensorField3D<T, 2> &velocity, MultiGridTensorField3D<T, 3> &S, Box3D domain)
{
    plint envelopeWidth = 1;
    applyProcessingFunctional(
        new BoxStrainRateFunctional3D<T, 2>, domain, velocity, S, velocity.getReferenceLevel(),
        envelopeWidth);
}

template <typename T>
std::unique_ptr<MultiGridTensorField3D<T, 3> > computeStrainRate(
    MultiGridTensorField3D<T, 2> &velocity, Box3D domain)
{
...
template <typename T>
std::unique_ptr<MultiGridTensorField3D<T, 3> > computeBulkStrainRate(
    MultiGridTensorField3D<T, 2> &velocity, Box3D domain)
{
    MultiGridTensorField3D<T, 3> *S = new MultiGridTensorField3D<T, 3>(velocity, domain);
