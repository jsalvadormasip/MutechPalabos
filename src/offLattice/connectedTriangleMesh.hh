...
RawConnectedTriangleMesh<T> offToConnectedTriangleMesh(OFFreader<T> const& reader)
{
    ...
    for (pluint iTriangle = 0; iTriangle < triangles.size(); iTriangle++) {
        PLB_ASSERT(facets[iTriangle].size() == 3); // The surface in the OFF file must be triangulated.
...
template<typename T>
ManualConnectedTriangleMesh<T>::ManualConnectedTriangleMesh(plint numTriangles_, plint numVertices_)
...
plint ManualConnectedTriangleMesh<T>::partId(std::string partName) const {
    std::vector<std::string>::const_iterator it = find(nameOfParts.begin(), nameOfParts.end(), partName);
...
template<typename T>    
typename RawConnectedTriangleMesh<T>::Vertex*
    RawConnectedTriangleMesh<T>::Vertex::clone() const
{
...
Array<T,3> RawConnectedTriangleMesh<T>::Vertex::normal(bool areaWeighted) const
{
    ...
    for (pluint i=0; i<nbTriangles.size(); ++i) {
        n += areaWeighted ?
            Triangle(mesh, nbTriangles[i]).normalTimesArea() : 
...
bool RawConnectedTriangleMesh<T>::Vertex::isInterior() const
{
    ...
    for (pluint i=0; i<neighbors.size(); ++i) {
        std::vector<plint> neighborNeighbors =
            mesh->vertex(neighbors[i])->adjacentVertices();
        pluint nearbyNeighbors = 0;
        for (pluint j=0; j<neighborNeighbors.size(); ++j) {
            typename std::set<T>::const_iterator it =
                neighborSet.find(neighborNeighbors[j]);
...
template<typename T>    
RawConnectedTriangleMesh<T>::Triangle::Triangle(RawConnectedTriangleMesh<T>* mesh_, plint triangle_)
    : mesh(mesh_),
      triangle(triangle_)
{ }

template<typename T>    
typename RawConnectedTriangleMesh<T>::Triangle*
    RawConnectedTriangleMesh<T>::Triangle::clone() const
{
    return new typename RawConnectedTriangleMesh<T>::Triangle(*this);
}

template<typename T>    
typename RawConnectedTriangleMesh<T>::CPVertex
    RawConnectedTriangleMesh<T>::Triangle::vertex(plint iVertex) const
{
...
template<typename T>    
typename RawConnectedTriangleMesh<T>::PVertex
    RawConnectedTriangleMesh<T>::Triangle::vertex(plint iVertex)
{
    PLB_ASSERT(iVertex<3);
    PLB_ASSERT(triangle < (plint)mesh->triangles.size());
...
T RawConnectedTriangleMesh<T>::Triangle::area() const {
    return computeTriangleArea (
        mesh->vertices[mesh->triangles[triangle][0]],
...
template<typename T>    
RawConnectedTriangleMesh<T>::AllTriangleIterator::AllTriangleIterator(RawConnectedTriangleMesh<T>* mesh_)
    : mesh(mesh_),
      currentTriangle(0)
{ }

template<typename T>    
typename RawConnectedTriangleMesh<T>::PTriangle RawConnectedTriangleMesh<T>::AllTriangleIterator::next()
{
...
template<typename T>    
RawConnectedTriangleMesh<T>::PartTriangleIterator::PartTriangleIterator(RawConnectedTriangleMesh<T>* mesh_, plint partId_)
    : mesh(mesh_),
      partId(partId_),
      currentTriangle(0)
{
...
template<typename T>    
typename RawConnectedTriangleMesh<T>::PTriangle RawConnectedTriangleMesh<T>::PartTriangleIterator::next()
{
    PLB_ASSERT( partId < (plint)mesh->trianglesPerPart.size() );
...
RawConnectedTriangleMesh<T>::RawConnectedTriangleMesh (
    ...
    : ManualConnectedTriangleMesh<T> (
            (plint)triangles_.size(),(plint)vertices_.size(),
            partTagging, nameOfParts ),
            vertices(vertices_),
            triangles(triangles_)
{
    computeTrianglesOnVertex();
...
template<typename T>
RawConnectedTriangleMesh<T> RawConnectedTriangleMesh<T>::merge(RawConnectedTriangleMesh<T>& rhs, T eps)
{
    ...
    RawConnectedTriangleMesh<T> newMesh = MeshConnector<T>(mergedRawTriangleMesh).generateConnectedMesh();
    return newMesh;
}
...
RawConnectedTriangleMesh<T> RawConnectedTriangleMesh<T>::refine(T eps) const
{
    ...
    RawConnectedTriangleMesh<T> newMesh = MeshConnector<T>(refinedRawTriangleMesh).generateConnectedMesh();
    return newMesh;
}
...
{
    RawTriangleMesh<T> rawTriangleMesh((TriangleMesh<T>&) *this, eps);
    RawTriangleMesh<T> refinedRawTriangleMesh = rawTriangleMesh.refineRecursively (
                                                    targetMaxEdgeLength, maxNumIterations, success );
    RawConnectedTriangleMesh<T> newMesh = MeshConnector<T>(refinedRawTriangleMesh).generateConnectedMesh();
    return newMesh;
}

template<typename T>
RawConnectedTriangleMesh<T> RawConnectedTriangleMesh<T>::select(TriangleSelector<T> const& selector, T eps) const
{
    RawTriangleMesh<T> rawTriangleMesh((TriangleMesh<T>&) *this, eps);
    RawTriangleMesh<T> subRawTriangleMesh = rawTriangleMesh.select(selector);
    RawConnectedTriangleMesh<T> newMesh = MeshConnector<T>(subRawTriangleMesh).generateConnectedMesh();
    return newMesh;
}

template<typename T>
RawConnectedTriangleMesh<T> RawConnectedTriangleMesh<T>::cutWithPlane(Array<T,3> planePos, Array<T,3> normal, T eps) const
{
    RawTriangleMesh<T> rawTriangleMesh((TriangleMesh<T>&) *this, eps);
...
template<typename T>
typename RawConnectedTriangleMesh<T>::PTriangleIterator RawConnectedTriangleMesh<T>::triangleIterator(plint partId)
{
...
template<typename T>
typename RawConnectedTriangleMesh<T>::PTriangle RawConnectedTriangleMesh<T>::triangle(plint uniqueID)
{
...
void RawConnectedTriangleMesh<T>::computeMinMaxEdges() const {
    if (minMaxEdgeLengthValid) return; // Lazy evaluation.
...
void RawConnectedTriangleMesh<T>::computeMinMaxAreas() const {
    if (minMaxTriangleAreaValid) return; // Lazy evaluation.
...
void RawConnectedTriangleMesh<T>::computeBoundingCuboid() const {
    if (boundingCuboidValid) return;  // Lazy evaluation.
    T xMin, yMin, zMin;
    T xMax, yMax, zMax;
...
RawConnectedTriangleMesh<T> disconnectedMerge (
        std::vector<RawConnectedTriangleMesh<T> const*> parts, std::vector<std::string> names, std::string vertexTagForParts )
{
    PLB_ASSERT( parts.size() == names.size() );
    // First, join all vertices and triangles of the provided parts, and create a RawConnectedTriangleMesh.
