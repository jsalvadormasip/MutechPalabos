...
ELIModels3D<T, Descriptor>::ELIModels3D(BoundaryShape3D<T, Array<T, 3>> *shape_, int flowType_) :
...
{
    auto &lattice = dynamic_cast<BlockLattice3D<T, Descriptor> &>(nonTypeLattice);
    auto *info = dynamic_cast<OffLatticeInfo3D *>(container.getData());
    PLB_ASSERT(info);
    std::vector<Dot3D> const &dryNodes = info->getDryNodes();
    std::vector<std::vector<int>> const &dryNodeFluidDirections = info->getDryNodeFluidDirections();
    ...
    Dot3D const &absoluteOffset, Array<T, 3> &localForce, std::vector<AtomicBlock3D *> const &args)
{
    ...
    for (plint iDirection = 0; iDirection < (plint)dryNodeFluidDirections.size(); ++iDirection) {
        ...
        auto [f_rhoBar, f_j] = getRhoBarJ(lattice, guoNode, args, fluidDirection, cellF);
        ...
        cellF[i_fluid] =
            0.5 * (alphaPlus + alphaMinus) * cellS[i_solid]
            + (1. + 0.5 * (alphaPlus - alphaMinus)) * cellF[i_fluid] /* = cellFF[i_fluid]*/
            + beta * cellF[i_solid] + Kplus * (fPlus - eqPlus) / (-1. + tauPlus)
            + Kmin * (fMinus - eqMinus) / (-1. + tauMinus) - alphaPlus * eqPlus - eqMinusWall;
    ...
    const Cell<T, Descriptor> &cellF) const
{
    ...
    if (args.empty()) {
        ...
        if ((plint)args.size() == 1) {
            auto const *macroField = dynamic_cast<NTensorField3D<T> const *>(args[0]);
            ...
            // #ifdef PLB_DEBUG
            auto const *rhoBarField = dynamic_cast<ScalarField3D<T> const *>(args[0]);
            // #endif
            auto const *jField = dynamic_cast<TensorField3D<T, 3> const *>(args[1]);
...
std::array<T, 5> ELIULK3<T, D>::eliCoefficients(T q, T tauPlus, T tauMinus) const
{
    ...
    T LambdaPlus = tauPlus - 0.5;
    T Kmin =
        1. + alphaMinus * LambdaMinus - (alphaMinus * (q * q + LambdaPlus)) / (2. * LambdaPlus);
    T beta = 0.0;
    return {{alphaPlus, alphaMinus, beta, Kplus, Kmin}};
}
