...
template<typename T>    
typename RawTriangleMesh<T>::Vertex*
    RawTriangleMesh<T>::Vertex::clone() const
{
...
template<typename T>    
typename RawTriangleMesh<T>::Triangle*
    RawTriangleMesh<T>::Triangle::clone() const
{
    return new typename RawTriangleMesh<T>::Triangle(*this);
}

template<typename T>    
typename RawTriangleMesh<T>::CPVertex
    RawTriangleMesh<T>::Triangle::vertex(plint iVertex) const
{
...
template<typename T>    
typename RawTriangleMesh<T>::PVertex
    RawTriangleMesh<T>::Triangle::vertex(plint iVertex)
{
    PLB_ASSERT(iVertex<3);
...
template<typename T>    
typename RawTriangleMesh<T>::PTriangleIterator
    RawTriangleMesh<T>::AllTriangleIterator::clone() const
{
...
template<typename T>    
RawTriangleMesh<T>::PartTriangleIterator::PartTriangleIterator(RawTriangleMesh<T>* mesh_, plint partId_)
    : mesh(mesh_),
      partId(partId_),
      currentTriangle(0)
{ }
...
template<typename T>    
typename RawTriangleMesh<T>::PTriangleIterator
    RawTriangleMesh<T>::PartTriangleIterator::clone() const
{
...
template<typename T>    
RawTriangleMesh<T>::RawTriangleMesh(std::vector<typename RawTriangleMesh<T>::RawTriangle> const& triangles_, T eps_)
    : eps(eps_)
{
    ...
    for (plint part=0; part<rhs.numParts(); ++part) {
        ...
        while (!it->end()) {
            ...
            nextTriangles.push_back (
                    RawTriangle( (*triangle)[0],(*triangle)[1],(*triangle)[2]) );
        }
...
template<typename T>
RawTriangleMesh<T> RawTriangleMesh<T>::refineRecursively(T targetMaxEdgeLength, plint maxNumIterations, bool& success) const
{
    ...
    for (plint iter=0; iter<maxNumIterations; ++iter) {
        ...
        if (result.getMaxEdgeLength() < targetMaxEdgeLength) break;
    }
...
template<typename T>
RawTriangleMesh<T> RawTriangleMesh<T>::refineByAreaRecursively(T targetMaxTriangleArea, plint maxNumIterations, bool& success) const
{
    ...
    for (plint iter=0; iter<maxNumIterations; ++iter) {
        ...
        if (result.getMaxTriangleArea() < targetMaxTriangleArea) break;
    }
...
RawTriangleMesh<T> RawTriangleMesh<T>::cutWithPlane(Array<T,3> planePos, Array<T,3> normal) const {
    ...
    PLB_ASSERT(!util::isZero(norm_normal)); // The cut plane normal vector cannot have zero magnitude.
    normal /= norm_normal;
    for (pluint iPart=0; iPart<triangles.size(); ++iPart) {
        ...
        for (pluint iTriangle = 0; iTriangle < triangles[iPart].size(); iTriangle++) {
            cutTriangleWithPlane(triangles[iPart][iTriangle], planePos, normal, tmpTriangles,
                    newZeroAreaTriangles);
        }
        tmpTriangles.insert(tmpTriangles.end(), newZeroAreaTriangles.begin(), newZeroAreaTriangles.end());
...
bool RawTriangleMesh<T>::areTheSameTriangle(RawTriangle const& t1, RawTriangle const& t2) const
{
    ...
    if (!util::isZero(n1020, eps) &&
        !util::isZero(n1021, eps) &&
        !util::isZero(n1022, eps)) {
        return false;
    }
    ...
    if (!util::isZero(n1120, eps) &&
        !util::isZero(n1121, eps) &&
        !util::isZero(n1122, eps)) {
        return false;
    }
    ...
    if (!util::isZero(n1220, eps) &&
        !util::isZero(n1221, eps) &&
        !util::isZero(n1222, eps)) {
        return false;
    }
...
template<typename T>
bool RawTriangleMesh<T>::containedAndErase(RawTriangle const& triangle, std::vector<RawTriangle>& triangles) const
{
...
bool RawTriangleMesh<T>::cutTriangleWithPlane (
        RawTriangle const& triangle, Array<T,3> const& planePos,
        Array<T,3> const& normal, std::vector<RawTriangle>& newTriangles,
        std::vector<RawTriangle>& newZeroAreaTriangles ) const
{
    ...
    for (int i = 0; i < 3; i++) {
        ...
        if (vertexTags[i] == 1 && vertexTags[j] == -1 && vertexTags[k] == -1) {
            ...
            int rv = 0;
            rv = lineIntersectionWithPlane<T>(Plane<T>(planePos, normal), triangle[i], triangle[j], eps, intersection_ij);
            if (rv != 1) {
                return false;
            }
            rv = lineIntersectionWithPlane<T>(Plane<T>(planePos, normal), triangle[i], triangle[k], eps, intersection_ik);
            if (rv != 1) {
                return false;
            }
            RawTriangle newTriangle;
    ...
    for (int i = 0; i < 3; i++) {
        ...
        if (vertexTags[i] == 0) {
            if (vertexTags[j] == 1 && vertexTags[k] == 1) {
                ...
                int rv = 0;
                rv = lineIntersectionWithPlane<T>(Plane<T>(planePos, normal), triangle[j], triangle[k], eps, intersection);
                if (rv != 1) {
                    return false;
                }
...
plint RawTriangleMesh<T>::partId(std::string partName) const {
    std::vector<std::string>::const_iterator it = find(partNames.begin(), partNames.end(), partName);
    if (it==partNames.end()) {
        return -1;
    }
    else {
    ...
    for (pluint part=0; part<triangles.size(); ++part) {
        for (pluint i=0; i<triangles[part].size(); i++) {
            for (plint j = 0; j < 3; j++) {
                triangles[part][i][j] =
                    plb::rotateAtOrigin (
                        triangles[part][i][j], normedAxis, theta );
            }
        }
    }
...
template<typename T>
T RawTriangleMesh<T>::getMinEdgeLength() const { return minEdgeLength; }

template<typename T>
T RawTriangleMesh<T>::getMaxEdgeLength() const { return maxEdgeLength; }

template<typename T>
T RawTriangleMesh<T>::getMinTriangleArea() const { return minTriangleArea; }

template<typename T>
T RawTriangleMesh<T>::getMaxTriangleArea() const { return maxTriangleArea; }

template<typename T>
Cuboid<T> RawTriangleMesh<T>::getBoundingCuboid() const { return boundingCuboid; }
