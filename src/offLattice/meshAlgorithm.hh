...
template<typename T>    
void toLatticeUnits(TriangleMesh<T>& mesh, Array<T,3> const& center, T dx, Array<T,3> const& offset) {
...
template<typename T>
void smooth(RawConnectedTriangleMesh<T>& triangleMesh, plint maxiter, T relax, bool isMeasureWeighted)
{
        ...
        for (plint iter = 0; iter < maxiter; iter++) {
            ...
            while (!vertexIterator->end()) {
                ...
                if (vertex->isInterior()) {
                    ...
                    (*bp1).vertex(iVertex)->get() = ((T) 1.0 - relax) * (*bp0).vertex(iVertex)->get() + relax * tmp / area;
                }
                else
                {  // vertex->isInterior()
                    ...
                    (*bp1).vertex(iVertex)->get() = ((T) 1.0 - relax) * (*bp0).vertex(iVertex)->get() +
        ...
        for (plint iter = 0; iter < maxiter; iter++) {
            ...
            while (!vertexIterator->end()) {
                ...
                if (vertex->isInterior()) {
                    ...
                    (*bp1).vertex(iVertex)->get() = ((T) 1.0 - relax) * (*bp0).vertex(iVertex)->get() + relax * tmp / (T) sizeIds;
                }
                else  // vertex->isInterior()
                {
                    ...
                    (*bp1).vertex(iVertex)->get() = ((T) 1.0 - relax) * (*bp0).vertex(iVertex)->get() +
                                      relax * ((*bp0).vertex(jVertex)->get() + (*bp0).vertex(kVertex)->get()) / (T) 2.0;
                }
            }
            // Swap buffers.
            std::swap(bp0, bp1);
...
Array<T,3> centerOfMass(TriangleMesh<T>& mesh)
{
    ...
    while (!iterator->end()) {
        ...
        T currentVolume = ((*triangle)[0][0]*(*triangle)[1][1]*(*triangle)[2][2] -
                           ...
                           (*triangle)[2][0]*(*triangle)[1][1]*(*triangle)[0][2]) / (T)6;
        totalVolume += currentVolume;
        center[0] += (((*triangle)[0][0] + (*triangle)[1][0] + (*triangle)[2][0]) / (T)4) * currentVolume;
        center[1] += (((*triangle)[0][1] + (*triangle)[1][1] + (*triangle)[2][1]) / (T)4) * currentVolume;
        center[2] += (((*triangle)[0][2] + (*triangle)[1][2] + (*triangle)[2][2]) / (T)4) * currentVolume;
    }
