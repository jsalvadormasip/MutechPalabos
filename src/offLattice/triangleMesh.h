...
public:
    ...
    struct Vertex {
        ...
        virtual T area() const =0;      /// One third of the sum of the areas of the neighboring triangles.
        virtual plint numAdjacentTriangles() const =0; /// How many triangles share this vertex?
        virtual CPTriangle adjacentTriangle(plint iTriangle) const =0; /// Get any of the adjacent triangles.
        virtual PTriangle adjacentTriangle(plint iTriangle) =0;        /// Get any of the adjacent triangles.
        virtual bool isInterior() const =0; /// Check if the vertex is an interior, non-boundary vertex.
        virtual std::vector<plint> adjacentVertices() const =0; /// Return a list of adjacent vertices.
        virtual T property(plint whichProperty) const =0; /// Get a vertex property (to know its name, ask the mesh).
        virtual void setProperty(plint whichProperty, T value) =0;
        virtual plint tag(plint whichTag) const =0;  /// Get a vertex tag (to know its name, ask the mesh).
    ...
    struct Triangle {
        ...
        virtual Array<T,3> continuousNormal(Array<T,3> const& p, bool areaWeighted=false) const =0;
