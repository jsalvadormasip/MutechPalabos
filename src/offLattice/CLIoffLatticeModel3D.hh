...
template<typename T, template<typename U> class Descriptor>
ContainerBlockData*
    CLIoffLatticeModel3D<T,Descriptor>::generateOffLatticeInfo() const
{
        ...
        Array<T,3>& localForce, std::vector<AtomicBlock3D *> const& args )
{
    ...
    for(pluint i=0; i<solidDirections.size(); ++i) {
        ...
        PLB_ASSERT( ok );
        T q = AC * invAB[iPop];
        Cell<T,Descriptor>& cellS = lattice.get(boundaryNode.x + D::c[iPop][0], boundaryNode.y + D::c[iPop][1], boundaryNode.z + D::c[iPop][2]);
        Cell<T,Descriptor>& cellFF = lattice.get(boundaryNode.x - D::c[iPop][0], boundaryNode.y - D::c[iPop][1], boundaryNode.z - D::c[iPop][2]);
        if (bdType==OffBoundary::dirichlet) {
            ...
            if (hasFluidNeighbor[i]) {
                ...
                T a3 = -a1; // kbar-1
                T a2 = 1.0 - a1 - a3; // k1
                T f_minq = cellF[iPop] - 2. * u_ci * D::t[iPop] * D::invCs2;
                T f_1minq = cellS[iPop] - 2. * u_ci * D::t[iPop] * D::invCs2;
                T f_1plusq = cellFF[oppPop];
                cellF[oppPop] = a1 * f_minq + a2 * f_1minq + a3 * f_1plusq;
            }
            else {
                ++numUnknown;
