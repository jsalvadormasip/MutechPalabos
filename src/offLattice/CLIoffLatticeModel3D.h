...
#ifndef CLI_OFF_LATTICE_MODEL_3D_H
#define CLI_OFF_LATTICE_MODEL_3D_H
...
class CLIoffLatticeModel3D : public OffLatticeModel3D<T, Array<T, 3> > {
public:
    CLIoffLatticeModel3D(BoundaryShape3D<T, Array<T, 3> > *shape_, int flowType_);
    virtual CLIoffLatticeModel3D<T, Descriptor> *clone() const;
...
private:
    ...
    ///   populations.
    class CLIOffLatticeInfo3D : public ContainerBlockData {
    public:
        CLIOffLatticeInfo3D() : localForce(Array<T, 3>::zero()) { }
        ...
        virtual CLIOffLatticeInfo3D *clone() const
        {
            return new CLIOffLatticeInfo3D(*this);
        }
...
#endif  // CLI_OFF_LATTICE_MODEL_3D_H
