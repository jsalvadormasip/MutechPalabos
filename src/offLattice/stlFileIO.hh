...
template<typename T>
STLreader<T>::STLreader(std::string fname, T eps_, TriangleSelector<T>* selector)
    : eps(eps_)
{
...
bool STLreader<T>::readBinarySTL(FILE* fp, TriangleSelector<T>* selector)
{
    ...
    while (fread(buf, sizeof(char), 80, fp) == 80 &&
           fread(&nt, sizeof(unsigned int), 1, fp) == 1) {
        count++;
...
template<typename T>
void STLreader<T>::fixOrientation(typename STLreader<T>::Triangle& triangle, Array<T,3> const& n) const
{
...
template<typename T>
void writeAsciiSTL(TriangleMesh<T>& mesh, std::string fname, int numDecimalDigits, bool mainProcOnly)
{
...
template<typename T>
void writeMultiPartAsciiSTL(TriangleMesh<T>& mesh, std::string fname, int numDecimalDigits, bool mainProcOnly)
{
    if (!mainProcOnly || global::mpi().isMainProcessor()) {
        {
            typename TriangleMesh<T>::PTriangleIterator iterator(mesh.triangleIterator());
...
template<typename T>
void writeBinarySTL(typename TriangleMesh<T>::PTriangleIterator const& iterator, std::string fname, bool mainProcOnly)
{
    if (!mainProcOnly || global::mpi().isMainProcessor()) {
