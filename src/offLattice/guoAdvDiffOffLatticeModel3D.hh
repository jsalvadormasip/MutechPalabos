...
void GuoAdvDiffOffLatticeModel3D<T, Descriptor>::computeRhoBarJNeq(
    ...
    int depth, Array<T, 3> const &wallNode, T delta, Array<T, 2> wallData, OffBoundary::Type bdType,
    Array<T, 3> const &wallNormal, T &rhoBar, Array<T, Descriptor<T>::d> &jNeq) const
{
    if (!usesSecondOrder()) {
        depth = 1;
    }
