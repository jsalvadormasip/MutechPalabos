...
RawConnectedTriangleMesh<T> constructCylinder (
        ...
        T length, plint nAxial, plint nCirc, T eps = getEpsilon<T>() );
...
RawConnectedTriangleMesh<T> constructCylinder (
        ...
        std::vector<Array<T,3> >& inletPoints, T eps = getEpsilon<T>() );
...
template<typename T>
RawConnectedTriangleMesh<T> constructRectangle(T lx, T ly, plint nx, plint ny, T eps = getEpsilon<T>());

template<typename T>
RawConnectedTriangleMesh<T> constructStrip(std::vector<Array<T,3> > from, std::vector<Array<T,3> > to, T eps = getEpsilon<T>());

template<typename T>
RawConnectedTriangleMesh<T> constructAnnulus(Array<T,3> const& center, T innerRadius, T outerRadius, plint nCirc,
        T eps = getEpsilon<T>());
