...
int pointOnTriangle (
        ...
        Array<T,3>& intersection, Array<T,3>& normal, T& distance )
{
    ...
    if (flag == 0) { // For intersection with a line segment
        if (util::lessThan(t, (T) 0, precision<T>::eps1) || util::greaterThan(t, (T) 1, precision<T>::eps1)) {
            return 0;
        }
    }
...
bool segmentIntersectsTriangle (
        Array<T,3> const& p1, Array<T,3> const& p2, typename TriangleMesh<T>::Triangle const& triangle )
{
    Array<T,3> v0 = triangle[0];
    Array<T,3> v1 = triangle[1];
    Array<T,3> v2 = triangle[2];
    ...
    //   this creates an awkward ambiguity. To remove this ambiguity, the
    //   triangle is slightly shifted in direction of its normal vector.
    if (    ( util::isZero(t, precision<T>::eps1) )
         || ( util::isOne (t, precision<T>::eps1) )
       )
    {
        normal /= norm(normal);
        // Shift the vertex, and recompute everything that needs to be
    ...
    if (util::lessThan(u, (T) 0, precision<T>::eps1) || util::greaterThan(u,   (T) 1, precision<T>::eps1) ||
        util::lessThan(v, (T) 0, precision<T>::eps1) || util::greaterThan(v,   (T) 1, precision<T>::eps1) ||
        ...
        T& distance, bool& intersectionIsInside )
{
    ...
    intersectionIsInside = util::greaterEqual(u, (T) 0, precision<T>::eps1) && util::lessEqual(u, (T) 1, precision<T>::eps1);
}

template <typename T>
void distanceToTrianglePlane (
        Array<T,3> const& point, typename TriangleMesh<T>::Triangle const& triangle,
        T& distance, bool& intersectionIsInside, bool& pointIsBehind )
{
    Array<T,3> normal = triangle.normal();
    Array<T,3> intersection((T) 0, (T) 0, (T) 0);
    int flag = 2; // Line.
    intersectionIsInside =
        pointOnTriangle( point, point+normal, flag, triangle,
