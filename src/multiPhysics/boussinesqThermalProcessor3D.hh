    ...
    BlockLattice3D<T, TemperatureDescriptor> &temperature)
{
    ...
    Array<T, D::d> gravOverDeltaTemp(
        gravity * dir[0] / deltaTemp, gravity * dir[1] / deltaTemp, gravity * dir[2] / deltaTemp);

    for (plint iX = domain.x0; iX <= domain.x1; ++iX) {
        for (plint iY = domain.y0; iY <= domain.y1; ++iY) {
            for (plint iZ = domain.z0; iZ <= domain.z1; ++iZ) {
                // Velocity coupling
                T *u = temperature.get(iX + offset.x, iY + offset.y, iZ + offset.z)
                ...
                T localTemperature =
                    temperature.get(iX + offset.x, iY + offset.y, iZ + offset.z).computeDensity();
    ...
    for (plint iX = domain.x0; iX <= domain.x1; ++iX) {
        for (plint iY = domain.y0; iY <= domain.y1; ++iY) {
            for (plint iZ = domain.z0; iZ <= domain.z1; ++iZ) {
                ...
                *temperature.get(iX + offset.x, iY + offset.y, iZ + offset.z)
                     .getExternal(omegaOffset) = fluid.get(iX, iY, iZ).getDynamics().getOmega();
                *temperature.get(iX + offset.x, iY + offset.y, iZ + offset.z)
                     .getExternal(rhoBarOffset) = rhoBar;
                vel.to_cArray(temperature.get(iX + offset.x, iY + offset.y, iZ + offset.z)
                                  .getExternal(velocityOffset));
                piNeq.to_cArray(temperature.get(iX + offset.x, iY + offset.y, iZ + offset.z)
...
{
    return new CompleteBoussinesqThermalProcessor3D<T, FluidDescriptor, TemperatureDescriptor>(
        *this);
}
...
SmagorinskyBoussinesqThermalProcessor3D<T,FluidDescriptor,TemperatureDescriptor>::
        SmagorinskyBoussinesqThermalProcessor3D(T gravity_, T T0_, T deltaTemp_,
Array<T,FluidDescriptor<T>::d> dir_, T cSmagoFluid_, T cSmagoTemp_ ) :  gravity(gravity_), T0(T0_),
deltaTemp(deltaTemp_), dir(dir_), cSmagoFluid(cSmagoFluid_), cSmagoTemp(cSmagoTemp_), nu0(nu0_),
d0(d0_)
{
    // We normalize the direction of the force vector.
    T normDir = std::sqrt(VectorTemplate<T,FluidDescriptor>::normSqr(dir));
    ...
    for (plint iX=domain.x0; iX<=domain.x1; ++iX)
    {
        for (plint iY=domain.y0; iY<=domain.y1; ++iY)
        {
            for (plint iZ=domain.z0; iZ<=domain.z1; ++iZ)
            {
                ...
                //   You can compute it with the method computeDensity().
                T localTemperature =
temperature.get(iX+offset.x,iY+offset.y,iZ+offset.z).computeDensity(); const T diffT =
localTemperature - T0; for (pluint iD = 0; iD < D::d; ++iD)
                {
                    force[iD] = gravOverDeltaTemp[iD] * diffT;
                }
            }
...
{
    return new
SmagorinskyBoussinesqThermalProcessor3D<T,FluidDescriptor,TemperatureDescriptor>(*this);
}
