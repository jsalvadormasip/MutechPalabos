...
T fluidCompliance = 6.e-5;  // The fluid->particle force is equal to the velocity difference times
                            // the fluid-compliance.
...
public:
    ParticleInteractionForce(T forceAmplitude_, Array<T, 3> gravity_) :
        forceAmplitude(forceAmplitude_), gravity(gravity_)
    { }
    ...
    virtual void processGenericBlocks(Box3D domain, std::vector<AtomicBlock3D *> blocks)
    {
        ...
        for (pluint iParticle = 0; iParticle < particles.size(); ++iParticle) {
            ...
            if (nonTypeParticle->getTag() != 0) {  // Exclude the wall particles.
                ...
                Box3D neighborhood(
                    x - commEnvelope, x + commEnvelope, y - commEnvelope, y + commEnvelope,
                    z - commEnvelope, z + commEnvelope);
                neighbors.clear();
                T rCritical    = 2.0;
                ...
                for (pluint iNeighbor = 0; iNeighbor < neighbors.size(); ++iNeighbor) {
                    ...
                    if (rSqr < util::sqr(cutOffLength) && rSqr > 1.e-6) {
                        ...
                        // r<=rCritical. This is to aovid numerical instability due to huge forces
                        // when two particles come too close (for example in a badly designed
                        // initial condition).
                        if (rSqr > rCriticalSqr) {
                            ...
                            // Inside a radius of rCritical (in lattice units), the force is
                            // constant.
                            T rNorm = std::sqrt(rSqr);
                            force +=
                                r / (rNorm * rCritical * rCriticalSqr * rCriticalSqr * rCriticalSqr)
                                * forceAmplitude;
                        }
                    }
                }
...
void buildConeGeometry()
{
    ...
    triangleSet = constructCylinder<T>(
        Array<T, 3>(0., 0., 0.), 1., (T)coneFactor, (T)lengthRatio * 2., nAxial, nCirc);

    // Transform the surface geometry of the tube to more efficient data structures that are
    // internally used by palabos.
...
{
    inletCenter =
        computeBaryCenter(triangleBd->getMesh(), triangleBd->getInletOutlet(xDirection)[0]);
    outletCenter =
        computeBaryCenter(triangleBd->getMesh(), triangleBd->getInletOutlet(xDirection)[1]);
    radius = computeInnerRadius(triangleBd->getMesh(), triangleBd->getInletOutlet(xDirection)[0]);

    pcout << "Inlet center: " << inletCenter[0] << "," << inletCenter[1] << "," << inletCenter[2]
          << std::endl;
    pcout << "Outlet center: " << outletCenter[0] << "," << outletCenter[1] << ","
          << outletCenter[2] << std::endl;
...
void createLattices()
{
    ...
    profiles.setInletOutlet(
        new VelocityPlugProfile3D<T>(uInject), new DensityNeumannBoundaryProfile3D<T>());
    ...
    defineDynamics(
        *fluidLattice, voxelizedDomain->getVoxelMatrix(), fluidLattice->getBoundingBox(),
        new NoDynamics<T, DESCRIPTOR>, voxelFlag::outside);
    // Default-initialize at equilibrium with zero-velocity and fixed pressure.
    initializeAtEquilibrium(
        *fluidLattice, fluidLattice->getBoundingBox(), (T)1., Array<T, 3>((T)0., (T)0., (T)0.));
    // Execute all data processors once to start the simulation off with well-defined initial
    // values.
    ...
    MultiBlockManagement3D particleManagement(
        ...
        latticeManagement.getThreadAttribution().clone(), commEnvelope,
...
void setupCouplings()
{
    ...
    integrateProcessingFunctional(
        new FluidToParticleCoupling3D<T, DESCRIPTOR>(1.), fluidLattice->getBoundingBox(),
        particleFluidArg, 1);
...
int main(int argc, char *argv[])
{
    ...
    for (plint i = 0; i < maxIter; ++i) {
        ...
        // For efficiency reasons, stop iterating the fluid once it is assumed to have reached a
        // stationary state.
        if (i < maxFluidIter) {
            // Iterate the fluid.
        ...
        if (i % saveIter == 0 && i > startParticleIter) {
            ...
            writeSelectedParticleVtk<T, DESCRIPTOR>(
                *particles, fName, particles->getBoundingBox(), util::SelectLargerEqualInt(1));
        }
    }
}
