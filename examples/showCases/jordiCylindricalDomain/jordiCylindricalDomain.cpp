...
int main(int argc, char *argv[])
{
    ...
    TriangleSet<T> triangleSet("s817withcylinder.stl", DBL);
    ...
    pcout << "Number of inlets/outlets which were closed: "
          << boundary.getInletOutlet(yDirection).size() << std::endl;
    // pcout << "Hola chavalin" << std::endl;
    inletCenter = computeBaryCenter(boundary.getMesh(), boundary.getInletOutlet(yDirection)[0]);
    outletCenter = computeBaryCenter(boundary.getMesh(), boundary.getInletOutlet(yDirection)[1]);
    radius = computeInnerRadius(boundary.getMesh(), boundary.getInletOutlet(yDirection)[0]);
    ...
    profiles.defineInletOutletTags(boundary, yDirection);
    ...
    // Functional which injects particles with predefined probability from the specified injection
    // domain.
    integrateProcessingFunctional(
        new AnalyticalInjectRandomParticlesFunctionalPPRNG3D<T, DESCRIPTOR, CircularInjection>(
            particleTemplate, particleProbabilityPerCell,
            CircularInjection(injectionRadius, inletCenter), particles->getBoundingBox(), &seed),
        injectionDomain, particleArg, 0);

    // Definition of an absorbtion domain for the particles. The specific domain is very close to
    // the
    //   exit of the tube.
