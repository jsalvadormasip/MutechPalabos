...
// Data processor to apply the solid fraction to the cell density (that will be used for partial
// collision)
            ...
            for (plint iY = domain.y0; iY <= domain.y1; ++iY) {
                ...
                for (plint iZ = domain.z0; iZ <= domain.z1; ++iZ) {
                    ...
                    *cell.getExternal(Descriptor<T>::ExternalField::rhoBarBeginsAt) =
                        solidFraction.get(
                            posX, posY,
                            posZ);  // apply here the solid fraction value to the lattice density
                }
            }
        }
    }
...
public:
    ...
    virtual void processGenericBlocks(Box3D domain, std::vector<AtomicBlock3D *> fields)
    {
        ...
        for (plint iZ = domain.z0; iZ <= domain.z1; ++iZ) {
            ...
            // iZ + offset.z : absolute z position ; read until position of z slice starting for
            // this core (disregard previous z slices)
            for (size_t lineNb = 0; lineNb < int(iZ + offset.z - simParam.clotBeginZ * simParam.nz)
                                                 * (countNXY(simParam.clotGeometryFile))
                                    && line == "";
                 ++lineNb)
                    ...
                    for (plint iX = domain.x0; iX <= domain.x1; ++iX) {
                        ...
                        if (std::stod(valsF) == 0.)
                            ...
                            if (simParam.permeModel == "Davies")
                                ns = T(1.)
                                     / T(1.
                                         + 2. * pow(simParam.Rf0 * 1.e-9, 2.)
                                               / (16 * pow(std::stod(valsF), 1.5)
                                                  * (1 + 56 * pow(std::stod(valsF), 3.)))
                                               / (simParam.nu_LB * pow(simParam.dx, 2.)));
                            else if (simParam.permeModel == "Clague")
                                ns = T(1.)
                                     / T(1.
                                         + 2. * pow(simParam.Rf0 * 1.e-9, 2.) * 0.50941
                                               * pow(
                                                   pow(simParam.pi / T(4. * std::stod(valsF)), 0.5)
                                                       - 1.,
                                                   2.)
                                               * exp(-1.8042 * std::stod(valsF))
                                               / (simParam.nu_LB * pow(simParam.dx, 2.)));
                            else if (simParam.permeModel == "JJ")
                                ns = T(1.)
                                     / T(1.
                                         + 2. * pow(simParam.Rf0 * 1.e-9, 2.)
                                               * (3. / T(20 * std::stod(valsF)))
                                               * (-log(std::stod(valsF)) - 0.931)
                                               / (simParam.nu_LB * pow(simParam.dx, 2.)));
                            else  // Walsh by default, no rescaling
                                ns = std::stod(valsF);
                        }
                        clotSolidFraction.get(
                            iX + offsetSolidFraction.x, iY + offsetSolidFraction.y,
                            iZ + offsetSolidFraction.z) = ns * frac;
                        clotSolidFractionPhys.get(
                            iX + offsetSolidFractionPhys.x, iY + offsetSolidFractionPhys.y,
                            iZ + offsetSolidFractionPhys.z) = std::stod(valsF) * frac;

                        clotFlags.get(iX, iY, iZ) = plint(
                            simParam.p0 * std::stod(valsF) * frac * simParam.dx * simParam.dx
                            * simParam.dx * 1e27 * (2. / double(45.)));
...
public:
    ...
    virtual void processGenericBlocks(Box3D domain, std::vector<AtomicBlock3D *> fields)
    {
        ...
        ParticleField3D<T, Descriptor> &particleField =
            *dynamic_cast<ParticleField3D<T, Descriptor> *>(fields[0]);
        ...
        for (plint iX = domain.x0; iX <= domain.x1; ++iX) {
            for (plint iY = domain.y0; iY <= domain.y1; ++iY) {
                for (plint iZ = domain.z0; iZ <= domain.z1; ++iZ) {
                    if (clotFlags.get(iX + offsetFlags.x, iY + offsetFlags.y, iZ + offsetFlags.z)
                        > 0)  // if current position is a clot (flag > 0)
                    {
                        // ------------- "t" ----------------
                        std::vector<Particle3D<T, Descriptor> *>
                            foundParticles;  // vector that will contain the particles found in the
                                             // cellDomain
                        Box3D cellDomain;
                        if (simParam.interactNeighbor)
                            cellDomain = Box3D(
                                iX - 1, iX + 1, iY - 1, iY + 1, iZ - 1,
                                iZ + 1);  // cellDomain to check = current position
                        else
                            cellDomain = Box3D(
                                iX, iX, iY, iY, iZ, iZ);  // cellDomain to check = current position
                        particleField.findParticles(
                            cellDomain,
                            foundParticles);  // find the particles (if any) in cellDomain

                        if (foundParticles.size() != 0) {
                            plint effectiveAntiFnQty =
                                0;  // compute the effective nb of real particles at position
                            for (size_t i = 0; i < foundParticles.size(); ++i)
                                effectiveAntiFnQty +=
                                    foundParticles[i]
                                        ->getTag();  // tag contains remaining real nb of particles
                                                     // for each super particle
                            plint effectiveFnQty = clotFlags.get(
                                iX + offsetFlags.x, iY + offsetFlags.y,
                                iZ + offsetFlags.z);  // get the effective nb of clot particles at
                                                      // position

                            plint effectiveFnToRemove = util::roundToInt(
                                simParam.dt * simParam.k1
                                * (simParam.dx * simParam.dx * simParam.dx) * effectiveFnQty
                                * effectiveAntiFnQty);  // compute reaction equation for Fn (divide
                                                        // by V_elem to convert concentration to
                                                        // qty)
                            plint effectiveAntiFnToRemove = util::roundToInt(
                                simParam.dt * simParam.k2
                                * (simParam.dx * simParam.dx * simParam.dx) * effectiveFnQty
                                * effectiveAntiFnQty);  // compute reaction equation for antiFn

                            // ------------- "t+1" --------------
                            // apply eq for Fn
                            if (effectiveFnQty > effectiveFnToRemove)
                                clotFlags.get(
                                    iX + offsetFlags.x, iY + offsetFlags.y, iZ + offsetFlags.z) =
                                    effectiveFnQty - effectiveFnToRemove;
                            else {
                                clotFlags.get(
                                    iX + offsetFlags.x, iY + offsetFlags.y, iZ + offsetFlags.z) =
                                    flagNums::fibrinDestroyed;
                                clotSolidFraction.get(
                                    iX + offsetSF.x, iY + offsetSF.y, iZ + offsetSF.z) =
                                    0;  // set to fluid node
                                clotSolidFractionPhys.get(
                                    iX + offsetSFPhys.x, iY + offsetSFPhys.y, iZ + offsetSFPhys.z) =
                                    0;  // set to fluid node
                            }
                            // apply eq for anti-Fn
                            for (size_t i = 0;
                                 (i < foundParticles.size() && effectiveAntiFnToRemove > 0);
                                 ++i)  // check all the particles found
                            {
                                if (effectiveAntiFnToRemove < foundParticles[i]->getTag()) {
                                    foundParticles[i]->setTag(
                                        foundParticles[i]->getTag() - effectiveAntiFnToRemove);
                                    effectiveAntiFnToRemove = 0;
                                }

                                else  // if more effectiveAntiFnToRemove than antiFn available in
                                      // i-th particle : remove that particle and update
                                      // effAntToRemove
                                {
            ...
            for (plint iY = domain.y0; iY <= domain.y1; ++iY) {
                for (plint iZ = domain.z0; iZ <= domain.z1; ++iZ) {
                    ...
                    L_x_t.get(iX + offsetL.x, iY + offsetL.y, iZ + offsetL.z) =
                        (clotFlagsIni.get(
                             iX + offsetFlagsIni.x, iY + offsetFlagsIni.y, iZ + offsetFlagsIni.z)
                         - clotFlagsCurr.get(
                             iX + offsetFlagsCurr.x, iY + offsetFlagsCurr.y,
                             iZ + offsetFlagsCurr.z))
                        * 1e3
                        / double(
                            simParam.dx * simParam.dx * simParam.dx
                            * simParam.avogadro);  // convert from qty of matter to µM

                    // L_DV/DV = 0 if clot empty (clotFlagsCurr = 0), then sF = 0
                    double LDV_DV =
                        clotFlagsIni.get(
                            iX + offsetFlagsCurr.x, iY + offsetFlagsCurr.y, iZ + offsetFlagsCurr.z)
                        / double(
                            simParam.dx * simParam.dx * simParam.dx * 1e6 * simParam.p0
                            * (2. / (double)45) * simParam.pi * simParam.Rf0 * simParam.Rf0);
                    // DV (cm^3)
                    ...
                    if (clotFlagsIni.get(
                        ...
                        R_x_t.get(iX + offsetR.x, iY + offsetR.y, iZ + offsetR.z) = sqrt(
                            simParam.Rf0 * simParam.Rf0
                            - double(1. / double(simParam.pi * simParam.p0))
                                  * L_x_t.get(iX + offsetL.x, iY + offsetL.y, iZ + offsetL.z)
                                  / ((LDV_DV) * double(1. / double(simParam.avogadro))
                                     * ((double)2e6 / (double)45e-3)));

                    // solid fraction
                    sFTmp = simParam.pi
                            * pow(R_x_t.get(iX + offsetR.x, iY + offsetR.y, iZ + offsetR.z), 2)
                            * (LDV_DV)*1e-21;
                    sFPhys_x_t.get(iX + offsetsFPhys.x, iY + offsetsFPhys.y, iZ + offsetsFPhys.z) =
                        sFTmp;
                    ...
                    if (sFTmp == 0.)
                        ...
                        if (simParam.permeModel == "Davies")
                            sF_x_t.get(iX, iY, iZ) =
                                ...
                                / T(1.
                                    + 2. * pow(simParam.Rf0 * 1.e-9, 2.)
                                          / (16 * pow(sFTmp, 1.5) * (1 + 56 * pow(sFTmp, 3.)))
                                          / (simParam.nu_LB * pow(simParam.dx, 2.)));
                        else if (simParam.permeModel == "Clague")
                            sF_x_t.get(iX, iY, iZ) =
                                T(1.)
                                / T(1.
                                    + 2. * pow(simParam.Rf0 * 1.e-9, 2.) * 0.50941
                                          * pow(pow(simParam.pi / T(4. * sFTmp), 0.5) - 1., 2.)
                                          * exp(-1.8042 * sFTmp)
                                          / (simParam.nu_LB * pow(simParam.dx, 2.)));
                        else if (simParam.permeModel == "JJ")
                            sF_x_t.get(iX, iY, iZ) =
                                T(1.)
                                / T(1.
                                    + 2. * pow(simParam.Rf0 * 1.e-9, 2.) * (3. / T(20 * sFTmp))
                                          * (-log(sFTmp) - 0.931)
                                          / (simParam.nu_LB * pow(simParam.dx, 2.)));
                        else  // Walsh by default, no rescaling
...
public:
    ...
    Clot(
        MultiBlockLattice3D<T, DESCRIPTOR> &lattice_, plint zBegin_ = 0.0, plint zEnd_ = 1.0,
        T porosity_ = 0.5)
    {
        lattice = &lattice_;
        zBegin = zBegin_;
    ...
    void generateFibrin(
        MultiScalarField3D<plint> &flags, MultiScalarField3D<double> &clotSolidFraction,
        MultiScalarField3D<double> &clotSolidFractionPhys, double frac)
    {
        ...
        // define domain before data processor, so parallelization can be made properly (domain
        // divided to cores)
        Box3D clotDomain(
            latticeDomain.x0, latticeDomain.x1, latticeDomain.y0, latticeDomain.y1, zBegin, zEnd);
        ...
        applyProcessingFunctional(
            new readClotAnaFromFile<double, DESCRIPTOR>(frac), clotDomain, clotFlagArg);

        std::vector<MultiBlock3D *> latticeClotArg;
        latticeClotArg.push_back(lattice);
        latticeClotArg.push_back(&clotSolidFraction);
        applyProcessingFunctional(
            new ApplySolidFractionToRhoBar<double, DESCRIPTOR>(), clotDomain, latticeClotArg);
    }
    ...
    void interact(std::vector<MultiBlock3D *> particleFlagSFArg, plint iter)
    {
        ...
        MultiParticleField3D<DenseParticleField3D<T, DESCRIPTOR>> &particleField =
            *dynamic_cast<MultiParticleField3D<DenseParticleField3D<T, DESCRIPTOR>> *>(
                particleFlagSFArg[0]);
        ...
        applyProcessingFunctional(
            new clotParticleInteraction<double, DESCRIPTOR>(iter), clotDomain, particleFlagSFArg);

        nbDestroyed =
            initNbParticles - countParticles(particleField, particleField.getBoundingBox());

        if (nbDestroyed > 0)
            pcout << nbDestroyed << " super particle(s) destroyed at time " << iter * simParam.dt
                  << "(it=" << iter << ")..." << endl;

        MultiScalarField3D<T> &clotSolidFraction =
            *dynamic_cast<MultiScalarField3D<T> *>(particleFlagSFArg[2]);
        std::vector<MultiBlock3D *> latticeClotArg;
        latticeClotArg.push_back(lattice);
        latticeClotArg.push_back(&clotSolidFraction);
        applyProcessingFunctional(
