...
public:
    Pipe(
        Array<T, 3> center_ = {0, 0, 0}, plint radius_ = 0, plint length_ = 0,
        bool isSquarePipe_ = false) :
        center(center_), radius(radius_), length(length_), isSquarePipe(isSquarePipe_)
    { }
    ...
    virtual bool operator()(plint iX, plint iY, plint iZ) const
    {
        if (!isSquarePipe)
            return (
                util::sqr((T)iX - center[0]) + util::sqr((T)iY - center[1]) > util::sqr(radius)
                || (iZ < center[2]) || (iZ > center[2] + length));
        else
            return (
                util::sqr((T)iX - center[0]) > util::sqr(radius)
                || util::sqr((T)iY - center[1]) > util::sqr(radius) || (iZ < center[2])
                || (iZ > center[2] + length));  // square pipe
    }
    ...
    const Array<T, 3> getCenter() const
    {
        return center;
    }
...
public:
    ...
    std::string clotGeometryFile =
        "clot_0.0003.txt";         // file from which to read the solidFraction of the clot
    ...
    // quantities for computation of Rf(x,t), for relation between qty of Fn and porosity locally
    // (Diamond and Anand)
    double Rf0 = 250;     // fibrin fiber radius nanometers
    double p0 = 0.01116;  // fibrin fiber density Fb/nm^2
    std::string permeModel =
        "Davies";  // gives relation k(ns*) (which uses value of Rf0). Options : Davies, Clague, JJ
                   // (Jackson-James), or None (gamma = ns)

    bool usesParticles = true;  // no particles = no fibrinolysis
    plint particlesNum = 100.;  // how many particles to inject initially (NB: doesn't affect
                                // concentration of anti-fibrin)
    double antiFnConcentration = 10.e-4;  // initial concentration of anti-fibrin, in mol/m³
                                          // (Franck's particle model). 1nM = 1e-6 mol/m³
    ...
    bool overwriteVtk = false;  // if true, simulation will only produce one set of vtk files at
                                // start of reaction, and one at the end (to reduce disk usage)
    ...
    double C_P;  // conversion factor for pressure : P = C_P*P_adim ; and rho_adim = P/(C_P*cs2) +
                 // rho_LB = P_adim/cs2 + rho_LB
    ...
    std::unique_ptr<MultiBlockLattice3D<T, DESCRIPTOR>> setupProblem(
        ...
        MultiScalarField3D<plint> &clotFlags, MultiScalarField3D<T> &clotSolidFraction,
        MultiScalarField3D<T> &clotSolidFractionPhys)
    {
        ...
        pcout << "domain : " << fullDomain.x1() - fullDomain.x0()
              << "; countNXY : " << (countNXY(clotGeometryFile))
        ...
        std::unique_ptr<MultiBlockLattice3D<T, DESCRIPTOR>> lattice =
            generateMultiBlockLattice<T, DESCRIPTOR>(
                Box3D(0, nx - 1, 0, ny - 1, 0, nz - 1), new PartialBBdynamics<T, DESCRIPTOR>(omega),
                commEnvelope);
        ...
        initializeAtEquilibrium(
            *(lattice.get()), lattice->getBoundingBox(),
            PoiseuilleDensityAndZeroVelocity<T>(
                poiseuillePipe, DeltaP / C_P) /*1., Array<T,3>(0,0,simParam.u)*/);

        // Bounce-Back walls for the pipe.
        defineDynamics(
            *(lattice.get()), lattice->getBoundingBox(), new Pipe<T>(poiseuillePipe),
        ...
        antiFnQtyPerCell =
            (double)antiFnConcentration * V0antiFn * avogadro
            / (double)particlesNum;  // but consider the whole volume before clot for initial antiFn
