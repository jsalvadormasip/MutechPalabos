...
public:
    CarreauFunction(T nu0_, T lambda_, T n_, T kappa_, T ly_, T tol_, int maxIter_) :
        nu0(nu0_), lambda(lambda_), n(n_), kappa(kappa_), ly(ly_), tol(tol_), maxIter(maxIter_)
    { }
...
public:
    ...
    virtual T derivative(T [[maybe_unused]] y, T z) const
    {
        T lz2 = lambda * lambda * z * z;

        return (T)4 * std::pow((T)1 + lz2, (n - (T)1) / (T)2) * ((n - (T)1) / (T)2) * lz2 * nu0
                   / ((T)1 + lz2)
