...
T computeU0(plint iX, plint iY, IncomprFlowParam<T> params)
{
    ...
    return (-(y - p1[1]) * std::exp(-(r1Sqr(x, y) / (r0 * r0)))
            + (y - p2[1]) * std::exp(-(r2Sqr(x, y) / (r0 * r0))))
           * omega_e / 2.0;
}
...
T computeV0(plint iX, plint iY, IncomprFlowParam<T> params)
{
    ...
    PLB_ASSERT(-1.0 <= x && x <= 1.0);
    PLB_ASSERT(-1.0 <= y && y <= 1.0);
    return ((x - p1[0]) * std::exp(-(r1Sqr(x, y) / (r0 * r0)))
            - (x - p2[0]) * std::exp(-(r2Sqr(x, y) / (r0 * r0))))
           * omega_e / 2.0;
}
...
template <typename T>
void geometrySetup(
    MultiGridLattice2D<T, DESCRIPTOR> &lattice, ConvectiveRefinementParameters<T> const &parameters,
    OnLatticeBoundaryCondition2D<T, DESCRIPTOR> &boundaryCondition)
{
    for (int iLevel = 0; iLevel < lattice.getNumLevels(); ++iLevel) {
        ...
        setBoundaryVelocity(
            lattice.getComponent(iLevel), lattice.getComponent(iLevel).getBoundingBox(),
            Array<T, 2>((T)0., (T)0.));
...
public:
    ...
    virtual void process(
        Box2D domain, BlockLattice2D<T, Descriptor> &lattice, ScalarField2D<T> &res);
...
template <typename T, template <typename U> class Descriptor>
void initializeLatticeWithDipoleVelocity(
...
std::unique_ptr<MultiScalarField2D<T> > solvePressureGS(MultiBlockLattice2D<T, Descriptor> &lattice)
{
    ...
    // initial pressure
    std::unique_ptr<MultiScalarField2D<T> > initialValue =
        multiply(DESCRIPTOR<T>::cs2, *add((T)-1., *density));
    // the result holder
...
void writeVTK(
    BlockLatticeT &lattice, IncomprFlowParam<T> const &parameters, plint iLevel, plint iter)
{
...
int main(int argc, char *argv[])
{
    ...
    ConvectiveRefinementParameters<T> parameters(numLevel, referenceLevel, baseParameters);

    // main object for the grid refinement
    plint overlapWidth = 1;
    MultiGridManagement2D management(
        baseParameters.getNx(), baseParameters.getNy(), numLevel, overlapWidth, behaviorLevel);

    Box2D refinementLevel0(
        0, 2 * N, 2 * N / 4, 2 * 3 * N / 4);  // refine the middle of the coarse domain
    Box2D refinementLevel1(
        4 * N / 3, 2 * N, 2 * N / 4 + N / 12,
        2 * 3 * N / 4 - N / 12);  // refine near the wall where the collision happens
    ...
    Dynamics<T, DESCRIPTOR> *backgroundDynamics =
        new BGKdynamics<T, DESCRIPTOR>(baseParameters.getOmega());
    ...
    if (computePoisson) {
        MultiBlockLattice2D<T, DESCRIPTOR> dummy(
            lattice.getComponent(0).getNx(), lattice.getComponent(0).getNy(),
        ...
        for (plint iLevel = 1; iLevel < numLevel; ++iLevel) {
            MultiBlockLattice2D<T, DESCRIPTOR> dummyLattice(
                lattice.getComponent(iLevel).getNx(), lattice.getComponent(iLevel).getNy(),
    ...
    for (plint iLevel = 0; iLevel < lattice.getNumLevels(); ++iLevel) {
        ...
        poisson >> press;
        std::unique_ptr<MultiTensorField2D<T, 2> > vel =
            computeVelocity(lattice.getComponent(iLevel));
    ...
    for (plint iT = 0; iT * baseParameters.getDeltaT() < maxT; ++iT) {
        ...
        if (iT % baseParameters.nStep(imSave) == 0) {
            ...
            // the finest one
            writeVTK(
                *lattice.convertToLevel(interpLevel), parameters[interpLevel], interpLevel, iT);
        ...
        if (iT * baseParameters.getDeltaT() >= t0 && iT * baseParameters.getDeltaT() <= t1) {
            MultiTensorField2D<T, 2> velocity =
                *computeVelocity(*lattice.convertToLevel(interpLevel));
            T e = computeEnstrophy(velocity)
                  / (parameters[numLevel - 1].getDeltaT() * parameters[interpLevel].getDeltaT());
        ...
        if (iT % baseParameters.nStep(logT) == 0) {
            MultiTensorField2D<T, 2> velocity =
                *computeVelocity(*lattice.convertToLevel(interpLevel));
            T e = computeEnstrophy(velocity)
                  / (parameters[interpLevel].getDeltaT() * parameters[interpLevel].getDeltaT());
