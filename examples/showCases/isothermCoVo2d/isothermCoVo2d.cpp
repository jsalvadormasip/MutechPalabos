...
public:
    ...
    Param(std::string xmlFname)
    {
        ...
        // dx = 0.01;                                // Space step in physical units [m] (now read
        // from .xml file  to avoid har coded value)
        ...
        //            = 343.20208332701009;
        // soundSpeed = 340.;                        // Simplified value (now read from .xml file to
        ...
        fnameBaseStr << std::setprecision(7)
                     << floor(1e6 * nu);  // Keeps only the first 6 decimals as an interger
        fnameBaseStr << "_";
        fnameBaseStr << N;
        fnameBaseStr << "_0_";
...
public:
    IsothermalCoVoInitialDensityAndVelocityField(Param<T> const &param_) : param(param_) { }
...
void simulationSetup(MultiBlockLattice2D<T, DESCRIPTOR> &lattice, const Param<T> &param)
{
    ...
    initializeAtEquilibrium(
        lattice, lattice.getBoundingBox(), IsothermalCoVoInitialDensityAndVelocityField<T>(param));
...
void writeVTS(MultiBlockLattice2D<T, DESCRIPTOR> &lattice, Param<T> &param, plint iter)
{
    T dx = param.dx;
    T dt = param.dt;
...
int main(int argc, char *argv[])
{
    ...
    if (argc != 5) {
        ...
        pcout << "Example: " << (std::string)global::argv(0) << " config.xml 1e-5 256 0.1"
              << std::endl;
        pcout << "Example (mpi): mpirun -np 2 " << (std::string)global::argv(0)
              << " config.xml 1e-5 256 0.1" << std::endl;
        exit(1);
    }
    ...
    for (plint iT = 0; iT <= tmax; ++iT) {
        ...
        if (iT % vtsTout == 0) {
            ...
            pcout << "Writing VTS file at t = " << iT / param.tc << " (iteration " << iT << ")"
                  << std::endl;
            // pcout << "maximal velocity = " << computeMax(*computeVelocityNorm(lattice)) <<
            // std::endl;
