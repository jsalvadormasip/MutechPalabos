    ...
    OnLatticeBoundaryCondition3D<T, DESCRIPTOR> &boundaryCondition, Array<plint, 3> &forceIds)
{
    ...
    boundaryCondition.addExternalVelocityEdge1NN(edge_bot_left, lattice, boundary::dirichlet);
    Box3D edge_top_left = Box3D(0, 0, 0, ny - 1, nz - 1, nz - 1);
    boundaryCondition.addExternalVelocityEdge1PN(edge_top_left, lattice, boundary::dirichlet);
    Box3D edge_bot_right = Box3D(nx - 1, nx - 1, 0, ny - 1, 0, 0);
    boundaryCondition.addExternalVelocityEdge1NP(edge_bot_right, lattice, boundary::neumann);
    Box3D edge_top_right = Box3D(nx - 1, nx - 1, 0, ny - 1, nz - 1, nz - 1);
    boundaryCondition.addExternalVelocityEdge1PP(edge_top_right, lattice, boundary::neumann);

    setBoundaryVelocity(
        lattice, lattice.getBoundingBox(), Array<T, 3>(parameters.getLatticeU(), 0.0, 0.0));
    // Box3D topLid = Box3D(0, nx-1, ny-1, ny-1, 0, nz-1);
    // Box3D everythingButTopLid = Box3D(0, nx-1, 0, ny-2, 0, nz-1);

    // boundaryCondition.setVelocityConditionOnBlockBoundaries(lattice, lattice.getBoundingBox(),
    // boundary::dirichlet);

    lattice.periodicity().toggle(1, true);

    initializeAtEquilibrium(
        lattice, lattice.getBoundingBox(), (T)1.,
        Array<T, 3>(parameters.getLatticeU(), (T)0., (T)0.));
...
void writeGifs(BlockLatticeT &lattice, IncomprFlowParam<T> const &parameters, plint iter)
{
    ...
    imageWriter.writeScaledGif(
        createFileName("ux", iter, 6), *computeVelocityComponent(lattice, slice, xComponent),
        imSize, imSize);

    imageWriter.writeScaledGif(
        createFileName("uNorm", iter, 6), *computeVelocityNorm(lattice, slice), imSize, imSize);
    imageWriter.writeScaledGif(
        createFileName("omega", iter, 6),
        *computeNorm(*computeVorticity(*computeVelocity(lattice)), slice), imSize, imSize);
}
