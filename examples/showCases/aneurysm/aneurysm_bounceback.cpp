...
std::vector<T> pointMeasures(
    MultiBlockLattice3D<T, DESCRIPTOR> &lattice, Array<T, 3> location, T dx, T dt)
{
    ...
    for (pluint i = 0; i < physicalPositions.size(); ++i) {
        ...
        if (performOutput) {
            pcout << "Pos (" << pos[0] << "," << pos[1] << "," << pos[2] << "); Velocity ("
                  << vel[0] << "," << vel[1] << "," << vel[2] << "); Vorticity (" << vort[0] << ","
                  << vort[1] << "," << vort[2] << "); Pressure " << pressure << std::endl;
        }
...
void writeImages(
    MultiBlockLattice3D<T, DESCRIPTOR> &lattice, MultiScalarField3D<int> &flagMatrix,
    Box3D const &imageDomain, Box3D const &vtkDomain, std::string fname, Array<T, 3> location, T dx,
    T dt)
{
    ...
    vtkOut.writeData<float>(
        *computeDensity(lattice, vtkDomain), "p", util::sqr(dx / dt) * fluidDensity);
    vtkOut.writeData<float>(*computeVelocityNorm(lattice, vtkDomain), "u", dx / dt);
    vtkOut.writeData<float>(
        *copyConvert<int, T>(*extractSubDomain(flagMatrix, vtkDomain)), "flag", 1.);
}

// This function produces images at predefined yz, xz and xy planes. The coordinates of the planes
// are given
//   in physical coordinates, and the output variables are velocity, vorticity and pressure.
void writeImages(
    MultiBlockLattice3D<T, DESCRIPTOR> &lattice, MultiScalarField3D<int> &flag, plint level,
    Array<T, 3> location, T dx, T dt)
{
    ...
    Box3D yz_imageDomain(
        util::roundToInt(lyz_plane[0]), util::roundToInt(lyz_plane[0]), 0, ny - 1, 0, nz - 1);
    Box3D xz_imageDomain(
        0, nx - 1, util::roundToInt(lxz_plane[1]), util::roundToInt(lxz_plane[1]), 0, nz - 1);
    Box3D xy_imageDomain(
        0, nx - 1, 0, ny - 1, util::roundToInt(lxy_plane[2]), util::roundToInt(lxy_plane[2]));

    Box3D yz_vtkDomain(
        util::roundToInt(lyz_plane[0]) - 3, util::roundToInt(lyz_plane[0]) + 3, 0, ny - 1, 0,
        nz - 1);
    Box3D xz_vtkDomain(
        0, nx - 1, util::roundToInt(lxz_plane[1]) - 3, util::roundToInt(lxz_plane[1]) + 3, 0,
        nz - 1);
    Box3D xy_vtkDomain(
        0, nx - 1, 0, ny - 1, util::roundToInt(lxy_plane[2]) - 3,
        util::roundToInt(lxy_plane[2]) + 3);

    writeImages(
        lattice, flag, xy_imageDomain, xy_vtkDomain, "xy_" + util::val2str(level), location, dx,
        dt);
    writeImages(
        lattice, flag, xz_imageDomain, xz_vtkDomain, "xz_" + util::val2str(level), location, dx,
        dt);
    writeImages(
        lattice, flag, yz_imageDomain, yz_vtkDomain, "yz_" + util::val2str(level), location, dx,
        dt);
}
    ...
    plint level, MultiBlockLattice3D<T, DESCRIPTOR> *iniVal = 0)
{
    ...
    if (useIncompressible) {
        dynamics =
            new IncBGKdynamics<T, DESCRIPTOR>(omega);  // In this model velocity equals momentum.
    } else {
    ...
    Box3D inletDomain(
        util::roundToInt(inletPos[0] - diameter), util::roundToInt(inletPos[0] + diameter),
    ...
    Box3D behindInlet(
        inletDomain.x0, inletDomain.x1, inletDomain.y0, inletDomain.y1, inletDomain.z0 - diameter,
        inletDomain.z0 - 1);

    Box3D outlet1Domain(
        util::roundToInt(outlet1Pos[0] - diameter), util::roundToInt(outlet1Pos[0] + diameter),
    ...
    Box3D behindOutlet1(
        outlet1Domain.x0, outlet1Domain.x1, outlet1Domain.y0, outlet1Domain.y1,
        outlet1Domain.z0 - diameter, outlet1Domain.z0 - 1);

    Box3D outlet2Domain(
        util::roundToInt(outlet2Pos[0]), util::roundToInt(outlet2Pos[0]),
    ...
    Box3D behindOutlet2(
        outlet2Domain.x0 - diameter, outlet2Domain.x1 - 1, outlet2Domain.y0, outlet2Domain.y1,
    ...
    defineDynamics(
        *lattice, flagMatrix, lattice->getBoundingBox(), new BounceBack<T, DESCRIPTOR>(1.), 0);
    ...
    pcout << "Saving a " << lattice->getNx() << " by " << lattice->getNy() << " by "
          << lattice->getNz() << " lattice." << std::endl;
...
void readParameters(XMLreader const &document)
{
    ...
    pcout << "Reynolds number, based on provided inlet diameter: "
          << averageInletVelocity * userDefinedInletDiameter / kinematicViscosity << std::endl;
    plbIOError(
        openingSortDirection < 0 || openingSortDirection > 2,
        ...
        for (plint level = iniLevel; level <= maxLevel; ++level) {
            ...
            if (level != maxLevel) {
                ...
                iniConditionLattice = std::unique_ptr<MultiBlockLattice3D<T, DESCRIPTOR> >(refine(
                    *convergedLattice, dxScale, dtScale, new BGKdynamics<T, DESCRIPTOR>(1.)));
            }
        }
