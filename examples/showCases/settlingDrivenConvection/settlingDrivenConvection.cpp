                ...
                for (plint iY=domain.y0; iY<=domain.y1; ++iY) {
                    for (plint iZ=domain.z0; iZ<=domain.z1; ++iZ) {
                        plint absoluteZ = absoluteOffset.z + iZ;
                          ...
                          T rand_val = dist(mt);
                          T VolFrac;
                          if(absoluteZ<(nz-1)-util::roundToInt(((up/(up+low))*nz))) VolFrac=0.0;
                          else VolFrac = rand_val;
                          if(absoluteZ>0.95*(nz-1)) VolFrac = rand_val*0.5*(1-erf((absoluteZ-0.98*(nz-1))/6));
...
int main(int argc, char *argv[])
{
    ...
    MultiScalarField3D<T> volfracField(MultiBlockManagement3D (
        blockStructure,
    ...
    MultiScalarField3D<T> T_t(volfracField), T_tp1(volfracField), Q(volfracField), v_sedimentation(volfracField);
    MultiScalarField3D<T> volfracField_RK(volfracField), phi_1(volfracField), phi_2(volfracField), phi_n_adv(volfracField), phi_1_adv(volfracField), phi_2_adv(volfracField);
    MultiScalarField3D<T> D_t(densityField), D_tp1(densityField), Q_d(densityField);

    MultiTensorField3D<T, 3> velocity(MultiBlockManagement3D (
        blockStructure,
        defaultMultiBlockPolicy3D().getThreadAttribution(),
        envelopeWidth ),
    defaultMultiBlockPolicy3D().getBlockCommunicator(),
...
//////////////////////////////////////////////////////////////////////////////////////////////
        ...
        cycle.addProcessor (
            new CopyConvertScalarFunctional3D<T,T>(),
            densityField_ID, D_tp1_ID,
            densityField.getBoundingBox() );
        cycle.addCommunication(D_tp1_ID, modif::staticVariables);
        cycle.addProcessor (
    	    new AdvectionDiffusionFd3D_neumann<T>(Di*parameters.getLatticeKappa(), upwind, neumann, nx, ny, nz),
...
//////////////////////////////////////////////////////////////////////////////////////////////
        ...
        cycle.addProcessor (
          new WENO3<T>(Di*parameters.getLatticeKappa(), eps, neumann, nx, ny, nz),
            arg1, volfracField.getBoundingBox() );
        cycle.addCommunication(phi_n_adv_ID, modif::staticVariables);
        cycle.addProcessor (
          new RK3_Step1_functional3D<T>(),
            volfracField_ID, phi_n_adv_ID, phi_1_ID,
           volfracField.getBoundingBox() );
        ...
        cycle.addProcessor (
          new RK3_Step2_functional3D<T>(),
            volfracField_ID, phi_1_ID, phi_1_adv_ID, phi_2_ID,
           volfracField.getBoundingBox() );
        cycle.addCommunication(phi_1_ID, modif::staticVariables);
        cycle.addProcessor (
            new CopyConvertScalarFunctional3D<T,T>(),
        ...
        cycle.addProcessor (
          new RK3_Step3_functional3D<T>(),
            volfracField_ID, phi_2_ID, phi_2_adv_ID, volfracField_RK_ID,
           volfracField.getBoundingBox() );
        cycle.addCommunication(volfracField_RK_ID, modif::staticVariables);
        cycle.addProcessor (
            new CopyConvertScalarFunctional3D<T,T>(),
            volfracField_RK_ID, volfracField_ID,
            volfracField.getBoundingBox() );
