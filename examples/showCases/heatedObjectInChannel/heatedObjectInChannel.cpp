...
struct Param {
    ...
    Param(std::string xmlFname)
    {
        ...
        PLB_ASSERT(
            precisionStr == "FLT" || precisionStr == "DBL" || precisionStr == "LDBL"
            || precisionStr == "INF");
        if (precisionStr == "FLT") {
            precision = FLT;
...
T poiseuilleVelocity(plint iY, plint iZ, plint maxN)
{
    ...
    for (plint iN = 0; iN < maxN; iN += 2) {
        ...
        sum +=
            (std::cos(twoNplusOne * pi * y / a) * std::cosh(twoNplusOne * pi * z / a)
    ...
    OnLatticeBoundaryCondition3D<T, DESCRIPTOR> &boundaryCondition)
{
    ...
    boundaryCondition.setVelocityConditionOnBlockBoundaries(
        lattice, param.inlet(), boundary::dirichlet);
    setBoundaryVelocity(lattice, param.inlet(), SquarePoiseuilleVelocity<T>(C, NMAX));

    boundaryCondition.setVelocityConditionOnBlockBoundaries(
        lattice, param.bottom(), boundary::dirichlet);
    boundaryCondition.setVelocityConditionOnBlockBoundaries(
        lattice, param.top(), boundary::dirichlet);
    boundaryCondition.setVelocityConditionOnBlockBoundaries(
        lattice, param.lateral1(), boundary::dirichlet);
    boundaryCondition.setVelocityConditionOnBlockBoundaries(
        lattice, param.lateral2(), boundary::dirichlet);
    ...
    integrateProcessingFunctional(
        new FluidPressureOutlet3D<T, DESCRIPTOR, 0, +1>(), param.outlet(), lattice, 0);
...
void writeVTK(
    OffLatticeBoundaryCondition3D<T, DESCRIPTOR, Velocity> &fbc,
    OffLatticeBoundaryCondition3D<T, TEMPERATURE_DESCRIPTOR, Array<T, 2> > &tbc, plint iT,
    std::string appStr = "")
{
    VtkImageOutput3D<T> vtkOut_x(createFileName(appStr + "x_slice_", iT, PADD), param.dx);
    vtkOut_x.writeData<float>(
        *fbc.computeVelocityNorm(param.vtkSliceX()), "velocityNorm", param.dx / param.dt);
    vtkOut_x.writeData<3, float>(
        *fbc.computeVelocity(param.vtkSliceX()), "velocity", param.dx / param.dt);
    vtkOut_x.writeData<float>(
        *fbc.computePressure(param.vtkSliceX()), "pressure",
        param.dx * param.dx / (param.dt * param.dt));
    vtkOut_x.writeData<float>(
        *tbc.computeDensity(param.vtkSliceX(), param.T_wall), "temperature", 1.0);

    VtkImageOutput3D<T> vtkOut_y(createFileName(appStr + "y_slice_", iT, PADD), param.dx);
    vtkOut_y.writeData<float>(
        *fbc.computeVelocityNorm(param.vtkSliceY()), "velocityNorm", param.dx / param.dt);
    vtkOut_y.writeData<3, float>(
        *fbc.computeVelocity(param.vtkSliceY()), "velocity", param.dx / param.dt);
    vtkOut_y.writeData<float>(
        *fbc.computePressure(param.vtkSliceY()), "pressure",
        param.dx * param.dx / (param.dt * param.dt));
    vtkOut_y.writeData<float>(
        *tbc.computeDensity(param.vtkSliceY(), param.T_wall), "temperature", 1.0);

    VtkImageOutput3D<T> vtkOut_z(createFileName(appStr + "z_slice_", iT, PADD), param.dx);
    vtkOut_z.writeData<float>(
        *fbc.computeVelocityNorm(param.vtkSliceZ()), "velocityNorm", param.dx / param.dt);
    vtkOut_z.writeData<3, float>(
        *fbc.computeVelocity(param.vtkSliceZ()), "velocity", param.dx / param.dt);
    vtkOut_z.writeData<float>(
        *fbc.computePressure(param.vtkSliceZ()), "pressure",
        param.dx * param.dx / (param.dt * param.dt));
    vtkOut_z.writeData<float>(
        *tbc.computeDensity(param.vtkSliceZ(), param.T_wall), "temperature", 1.0);
}
...
void run(std::string continueFileName)
{
    ...
    DEFscaledMesh<T> *defMesh =
        new DEFscaledMesh<T>(*triangleSet, 0, xDirection, margin, Dot3D(0, 0, 0));
    delete triangleSet;
    triangleSet = 0;
    ...
    VoxelizedDomain3D<T> voxelizedDomain(
        triangleBoundary, flowType, param.bbox(), borderWidth, extendedEnvelopeWidth, blockSize);
    pcout << getMultiBlockInfo(voxelizedDomain.getVoxelMatrix()) << std::endl;
    {
        VtkImageOutput3D<T> vtkOut(outDir + "voxels_full_domain", param.dx);
        vtkOut.writeData<float>(
            *copyConvert<int, T>(voxelizedDomain.getVoxelMatrix(), param.bbox()), "voxel", 1.0);
    }
    ...
    if (param.useRhoBarJ) {
        lattice = generateMultiBlockLattice<T, DESCRIPTOR>(
                      voxelizedDomain.getVoxelMatrix(), envelopeWidth,
                      new NoDynamics<T, DESCRIPTOR>((T)1))
                      .release();
    } else {
    ...
    if (param.useRhoBarJ) {
        ...
        integrateProcessingFunctional(
            new PackedRhoBarJfunctional3D<T, DESCRIPTOR>(), lattice->getBoundingBox(), *lattice,
            *rhoBarJfield, 0);
    }
    ...
    MultiBlockLattice3D<T, TEMPERATURE_DESCRIPTOR> *temperatureLattice =
        new MultiBlockLattice3D<T, TEMPERATURE_DESCRIPTOR>(
            (MultiBlock3D &)voxelizedDomain.getVoxelMatrix());
    defineDynamics(
        *temperatureLattice, temperatureLattice->getBoundingBox(),
        new AdvectionDiffusionRLBdynamics<T, TEMPERATURE_DESCRIPTOR>(param.omega_temperature));
    defineDynamics(
        *temperatureLattice, voxelizedDomain.getVoxelMatrix(), temperatureLattice->getBoundingBox(),
    ...
    integrateProcessingFunctional(
        new LatticeToPassiveAdvDiff3D<T, DESCRIPTOR, TEMPERATURE_DESCRIPTOR>(
            (T)param.temperatureTimeFactor),
    ...
    if (continueSimulation) {
        ...
        initializeAtEquilibrium(
            *lattice, lattice->getBoundingBox(), SquarePoiseuilleDensityAndVelocity<T>(C, NMAX));
        lattice->initialize();
        initializeAtEquilibrium(
            *temperatureLattice, temperatureLattice->getBoundingBox(), param.T_inf,
    ...
    for (plint i = iter; i < param.maxIter && !stopProgram; ++i) {
        ...
        if (i % param.statIter == 0 || output) {
            ...
            if (i != 0) {
                pcout << "Time per lattice Boltzmann cycle: "
                      << global::timer("cycle").getTime() / (T)i << std::endl;
            }
            pcout << std::endl;
        }
        ...
        if ((param.cpIter > 0 && i % param.cpIter == 0 && i != iter) || output) {
            ...
            saveState(
                checkpointBlocks, i, param.saveDynamicContent, param.continueFile,
                param.checkpointFile, PADD);
            pcout << std::endl;
        }
        if (i % param.abIter == 0) {
            stopProgram = abortExecution(
                param.abortFile, checkpointBlocks, i, param.saveDynamicContent, param.continueFile,
                param.checkpointFile, PADD);
