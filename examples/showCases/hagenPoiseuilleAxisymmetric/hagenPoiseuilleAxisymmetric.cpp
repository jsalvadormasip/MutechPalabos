...
/** \file
 * Implementation of a stationary, pressure-driven axisymmetric Hagen-Poiseuille
 *flow, and comparison with the analytical Hagen-Poiseuille profile. The
 *velocity is initialized to zero, and converges only slowly to the expected
 *parabola. This application illustrates a full production cycle in a CFD
 *application, ranging from the creation of a geometry and definition of
 *boundary conditions over the program execution to the evaluation of results
 *and production of instantaneous graphical snapshots. From a technical
 *standpoint, this showcase is not trivial: it implements for example hypbrid
 *velocity/pressure boundaries, and uses an analytical profile to set up the
 *boundary and initial conditions, and to compute the error. As a first Palabos
 *example, you might prefer to look at a more straightforward code, such as
 *cavity2d.
 **/
