...
struct SimulationParameters {
    ...
    T rho_ShapeOp_RBC, Calpha_ShapeOp_RBC, Cbeta_ShapeOp_RBC,
        globalVolumeConservationWeight_ShapeOp_RBC;
    T rho_ShapeOp_PLT, Calpha_ShapeOp_PLT, Cbeta_ShapeOp_PLT,
        globalVolumeConservationWeight_ShapeOp_PLT;
    ...
    int loc_rank = 0, sock_rank, local_body_id = 0, total_devices_nb = 0, number_devices = 1,
        number_nodes = 1;
    // Poiseuille Flow
    T drivingForce_lb;
    T pipeRadius_LB;
...
// for debuging only
void flatten_mesh(
    RawConnectedTriangleMesh<double> *obstacle, double *obstacle_vertices, double *obstacle_normals)
{
...
// Shear Flow
void flowSetup(
    Group3D &group, OnLatticeBoundaryCondition3D<T, DESCRIPTOR> &boundaryCondition, T vel)
{
    ...
    if (sp.Simulation_Type.compare("Shear_Flow") == 0) {
        boundaryCondition.addVelocityBoundary1P(top, group.getLattice<T, DESCRIPTOR>("lattice"));
        boundaryCondition.addVelocityBoundary1N(bottom, group.getLattice<T, DESCRIPTOR>("lattice"));

        // Velocity in lattice units
        setBoundaryVelocity(
            group.getLattice<T, DESCRIPTOR>("lattice"), top, Array<T, 3>(0., 0., vel));
        setBoundaryVelocity(
            group.getLattice<T, DESCRIPTOR>("lattice"), bottom, Array<T, 3>(0., 0., 0.));
        initializeAtEquilibrium(
            group.getLattice<T, DESCRIPTOR>("lattice"), group.getBoundingBox(), 1.0,
            Array<T, 3>::zero());
        initializeAtEquilibrium(
            group.getLattice<T, DESCRIPTOR>("lattice"), top, 1.0, Array<T, 3>(0., 0., vel));
        initializeAtEquilibrium(
            group.getLattice<T, DESCRIPTOR>("lattice"), bottom, 1.0, Array<T, 3>(0., 0., 0.));
    } else {
        // rho_LB = 1.
        initializeAtEquilibrium(
            group.getLattice<T, DESCRIPTOR>("lattice"), group.getBoundingBox(), 1.,
            Array<T, 3>::zero());
    }
...
// units.
void iniShapeOpSolvers(
    std::string fname_points, std::string fname_constraints, std::string fname_connectivity,
    std::string fname_onsurface)
{
    ...
    sp.rbcTemplate =
        new RawConnectedTriangleMesh<T>(generateMesh<T>(sp.RBC_shapeOpSolverTemplate, sp.dx_p));
    sp.pltTemplate =
        new RawConnectedTriangleMesh<T>(generateMesh<T>(sp.PLT_shapeOpSolverTemplate, sp.dx_p));

    if (sp.wallStlFileName == "") {
        sp.wallMesh = nullptr;
    } else {
        pcout << "I am reading the wall: " << sp.wallStlFileName << std::endl;
        sp.wallMesh = new RawConnectedTriangleMesh<T>(
            stlToConnectedTriangleMesh<T>(sp.wallStlFileName, getEpsilon<T>(DBL)));
    ...
    MPI_Comm shmcomm;
    // MPI_COMM_TYPE_SHARED: the communicator will be split into subcommunicators whose processes
    // can create shared memory regions This is highly useful when you want to have a hierarchy of
    // communication where communicators are created based on node-level memory locality, even if a
    // shared memory region is not needed
    ...
    if (sp.numRBC > 0)
        util::linearRepartition(
            0, sp.numRBC - 1, std::min((pluint)sp.total_devices_nb, sp.numRBC), ranges_RBCs);

    if (sp.numPLT > 0)
        util::linearRepartition(
            sp.numRBC, sp.numPLT + sp.numRBC - 1, std::min((pluint)sp.total_devices_nb, sp.numPLT),
            ranges_PLTs);
    ...
    if (sp.numRBC > 0)
        util::linearRepartition(
            0, sp.numRBC - 1, std::min((pluint)numProc, sp.numRBC), ranges_RBCs);

    if (sp.numPLT > 0)
        util::linearRepartition(
            sp.numRBC, sp.numPLT + sp.numRBC - 1, std::min((pluint)numProc, sp.numPLT),
            ranges_PLTs);
    ...
    // for CPUs with no matched GPUs: { myRangeRBC.second - myRangeRBC.first + myRangePLT.second -
    // myRangePLT.first + 2 } == 0
    sp.shapeOpBodies = std::vector<ShapeOpBody<T>>(
        sp.myRangeRBC.second - sp.myRangeRBC.first + sp.myRangePLT.second - sp.myRangePLT.first
        + 2);
    ...
    auto initBody = [&](std::vector<ShapeOp_Solver *> &shapeOpP, std::string &case_name,
                        plb::RawConnectedTriangleMesh<T> *mesh_template, T Calpha_ShapeOp_,
                        T Cbeta_ShapeOp_, T rho_, T applyGlobalVolumeConservation_ShapeOp_,
                        T globalVolumeConservationWeight_ShapeOp_, std::vector<int> *mesh_graph,
                        int bodyID, int i, int body_type) {
        ...
        if (!sp.restartFromCP) {
            ...
            setConnectivityListFromCSV(
                *solver, "./Case_Studies/" + case_name + "/" + fname_connectivity);
            ...
            solver->initialize(
                Calpha_ShapeOp_, Cbeta_ShapeOp_, sp.dt_p * (T)(sp.timestep_ShapeOp), rho_, false,
                applyGlobalVolumeConservation_ShapeOp_, globalVolumeConservationWeight_ShapeOp_,
                sp.dx_p, sp.dt_p);
        } else {
            setConstraintsFromCSV(*solver, "./Case_Studies/" + case_name + "/" + fname_constraints);
            setConnectivityListFromCSV(
            ...
            if (!sp.continuePLB) {
                // setPointsFromCSV(*solver, CP_OutputDir + "body_ID_" + util::val2str(bodyID) +
                // ".csv");
                //  This is to avoid setting the initial conditions with deformed bodies such that
                //  we avoid energy bursts
                setPointsFromCSV(
                    *solver, sp.CP_OutputDir + "body_ID_" + util::val2str(bodyID) + ".csv", true);
            } else {
                setPointsFromCSV(
                    *solver, createFileName(
                                 sp.CP_OutputDir + "body_ID_" + util::val2str(bodyID) + "_iT_",
                                 sp.iT, sp.fileNamePadding)
                                 + ".csv");
                setVelsFromCSV(
                    *solver, createFileName(
                                 sp.CP_OutputDir + "vels_body_ID_" + util::val2str(bodyID) + "_iT_",
                                 sp.iT, sp.fileNamePadding)
                                 + ".csv");
            }
        }
    ...
    if (sp.shapeOpBodies.size()) {
        ...
        for (plint bodyID = sp.myRangeRBC.first; bodyID <= sp.myRangeRBC.second; ++bodyID) {
            initBody(
                sp.shapeOpRBCs, sp.Case_Study_RBC, sp.rbcTemplate, sp.Calpha_ShapeOp_RBC,
                sp.Cbeta_ShapeOp_RBC, sp.rho_ShapeOp_RBC,
                sp.applyGlobalVolumeConservation_ShapeOp_RBC,
                sp.globalVolumeConservationWeight_ShapeOp_RBC, rbc_graph, bodyID, sp.local_body_id,
                0);
            sp.local_body_id++;
        }
        ...
        for (plint bodyID = sp.myRangePLT.first; bodyID <= sp.myRangePLT.second; ++bodyID) {
            initBody(
                sp.shapeOpPLTs, sp.Case_Study_PLT, sp.pltTemplate, sp.Calpha_ShapeOp_PLT,
                sp.Cbeta_ShapeOp_PLT, sp.rho_ShapeOp_PLT,
                sp.applyGlobalVolumeConservation_ShapeOp_PLT,
                sp.globalVolumeConservationWeight_ShapeOp_PLT, plt_graph, bodyID, sp.local_body_id,
                1);
            sp.local_body_id++;
        }
    }
...
void iniPalabosParticles(Group3D &group, T vel)
{
    ...
    const std::vector<bool> &RBC_onSurfaceParticle =
        sp.RBC_shapeOpSolverTemplate.get_onSurfaceParticle();
    for (plint bodyID = sp.myRangeRBC.first; bodyID <= sp.myRangeRBC.second; ++bodyID) {
        ...
        if (!sp.restartFromCP) {
            ...
            if (!sp.continuePLB) {
                // setPointsFromCSV(shapeOpSolverTemplate_tmp, CP_OutputDir + "body_ID_" +
                // util::val2str(bodyID) + ".csv");
                //  This is to avoid setting the initial conditions with deformed bodies such that
                //  we avoid energy bursts
                setPointsFromCSV(
                    shapeOpSolverTemplate_tmp,
                    sp.CP_OutputDir + "body_ID_" + util::val2str(bodyID) + ".csv", true);
            } else {
                setPointsFromCSV(
                    shapeOpSolverTemplate_tmp,
                    createFileName(
                        sp.CP_OutputDir + "body_ID_" + util::val2str(bodyID) + "_iT_", sp.iT,
                        sp.fileNamePadding)
                        + ".csv");
                setVelsFromCSV(
                    shapeOpSolverTemplate_tmp,
                    createFileName(
                        sp.CP_OutputDir + "vels_body_ID_" + util::val2str(bodyID) + "_iT_", sp.iT,
                        sp.fileNamePadding)
                        + ".csv");
            }
        }
        ...
        for (pluint vertexID = 0; vertexID < (pluint)tmp_points.cols(); ++vertexID) {
            ...
            controlPositionPeriodic<T>(
                position, group.getLattice<T, DESCRIPTOR>("lattice").getNx() - 1,
                group.getLattice<T, DESCRIPTOR>("lattice").getNy() - 1,
                group.getLattice<T, DESCRIPTOR>("lattice").getNz() - 1, x, y, z);

            if (global::mpi().getRank() == 0) {
                // particle position in lattice units
                particles.push_back(
                    new bodyVertexParticle<T, Descriptor>(vertexID, position, bodyID, velocity));
            } else {
    ...
    const std::vector<bool> &PLT_onSurfaceParticle =
        sp.PLT_shapeOpSolverTemplate.get_onSurfaceParticle();
    for (plint bodyID = sp.myRangePLT.first; bodyID <= sp.myRangePLT.second; ++bodyID) {
    ...
    if (sp.wallMesh) {
        for (pluint vertexID = 0; vertexID < (pluint)sp.wallMesh->getNumVertices(); ++vertexID) {
                ...
                || sp.Simulation_Type.compare("Box_Poiseuille_Flow") == 0)
            {
                ...
                particles.push_back(new bodyVertexParticle<T, Descriptor>(
                    vertexID, position, IDforWall, Array<T, 3>::zero()));
            } else {
                if (position[1] < ((sp.ly_p / 2.) / sp.dx_p))  // bottom
                    particles.push_back(new bodyVertexParticle<T, Descriptor>(
                        vertexID, position, IDforWall, Array<T, 3>(0., 0., 0.)));
                else  // top
                    particles.push_back(new bodyVertexParticle<T, Descriptor>(
                        vertexID, position, IDforWall, Array<T, 3>(0., 0., vel)));
            }
    ...
    if (global::mpi().getRank() != 0)  // SEND
    {
        ...
        MPI_Isend(
            &containers_size, 1, MPI_UNSIGNED_LONG_LONG, 0, MPItagIniPalabosParticles,
            MPI_COMM_WORLD, mpiRequests.back());

        if (containers_size > 0) {
            mpiRequests.push_back(new MPI_Request);
            MPI_Isend(
                &verIDs[0], (int)containers_size, MPI_UNSIGNED_LONG_LONG, 0,
                MPItagIniPalabosParticles, MPI_COMM_WORLD, mpiRequests.back());

            mpiRequests.push_back(new MPI_Request);
            MPI_Isend(
                &ps[0][0], (int)(3 * containers_size), MPI_DOUBLE, 0, MPItagIniPalabosParticles,
                MPI_COMM_WORLD, mpiRequests.back());

            mpiRequests.push_back(new MPI_Request);
            MPI_Isend(
                &bdIDs[0], (int)containers_size, MPI_UNSIGNED_LONG_LONG, 0,
                MPItagIniPalabosParticles, MPI_COMM_WORLD, mpiRequests.back());

            mpiRequests.push_back(new MPI_Request);
            MPI_Isend(
                &vs[0][0], (int)(3 * containers_size), MPI_DOUBLE, 0, MPItagIniPalabosParticles,
                MPI_COMM_WORLD, mpiRequests.back());
        }
    } else  // RECV
    {
        ...
        while (received != (numProc - 1)) {
            ...
            MPI_Recv(
                &containers_size, 1, MPI_UNSIGNED_LONG_LONG, MPI_ANY_SOURCE,
            ...
            if (containers_size > 0) {
                ...
                MPI_Recv(
                    &verIDs_[0], (int)containers_size, MPI_UNSIGNED_LONG_LONG, sending_processor,
                    MPItagIniPalabosParticles, MPI_COMM_WORLD, &dummyStatus);

                MPI_Recv(
                    &ps_[0][0], (int)(3 * containers_size), MPI_DOUBLE, sending_processor,
                    MPItagIniPalabosParticles, MPI_COMM_WORLD, &dummyStatus);

                MPI_Recv(
                    &bdIDs_[0], (int)containers_size, MPI_UNSIGNED_LONG_LONG, sending_processor,
                    MPItagIniPalabosParticles, MPI_COMM_WORLD, &dummyStatus);

                MPI_Recv(
                    &vs_[0][0], (int)(3 * containers_size), MPI_DOUBLE, sending_processor,
                    MPItagIniPalabosParticles, MPI_COMM_WORLD, &dummyStatus);

                for (pluint l = 0; l < containers_size; ++l) {
                    particles.push_back(new bodyVertexParticle<T, Descriptor>(
                        verIDs_[l], ps_[l], bdIDs_[l], vs_[l]));
                }
            }
    ...
    injectParticlesAtMainProc<T, Descriptor>(
        particles, group.getDenseParticles<T, Descriptor>("vertexParticles"),
...
void time_statistics(pluint iter, pluint delta_iter)
{
    ...
    std::stringstream ss;
    ss << "At iteration " << std::setw(6) << std::left << iter << "the statistitics over the past "
       << std::setw(6) << std::left << delta_iter << "iterations is:\n";
    ss << "===================================================================="
          "==\n";
    ...
    ss << std::setw(6) << std::right << std::setprecision(3) << (t_reset_com / t_all) * 100.0
       << "%";
    ...
    ss << std::setw(6) << std::right << std::setprecision(3) << (t_mpi / t_all) * 100.0 << "%";
    ss << std::setw(8) << std::right << std::setprecision(3) << "(" << (t_mpi / delta_iter) * 1.e3
       << " ms/iT)\n";

    ss << std::setw(44) << std::left << "ShapeOp:";
    ss << std::setw(6) << std::right << std::setprecision(3) << (t_shapeop / t_all) * 100.0 << "%";
    ss << std::setw(8) << std::right << std::setprecision(3) << "("
       << (t_shapeop / delta_iter) * 1.e3 << " ms/iT)\n";

    ss << std::setw(44) << std::left << "apply-containers:";
    ss << std::setw(6) << std::right << std::setprecision(3) << (t_apply_cont / t_all) * 100.0
       << "%";
    ss << std::setw(8) << std::right << std::setprecision(3) << "("
       << (t_apply_cont / delta_iter) * 1.e3 << " ms/iT)\n";

    ss << std::setw(44) << std::left << "actions1 (BoxRhoBarJPiNeq):";
    ss << std::setw(6) << std::right << std::setprecision(3) << (t_actions1 / t_all) * 100.0 << "%";
    ss << std::setw(8) << std::right << std::setprecision(3) << "("
       << (t_actions1 / delta_iter) * 1.e3 << " ms/iT)\n";

    ss << std::setw(44) << std::left << "actions2a (LocMesh):";
    ss << std::setw(6) << std::right << std::setprecision(3) << (t_actions2a / t_all) * 100.0
       << "%";
    ss << std::setw(8) << std::right << std::setprecision(3) << "("
       << (t_actions2a / delta_iter) * 1.e3 << " ms/iT)\n";

    ss << std::setw(44) << std::left << "actions2b (Combo):";
    ss << std::setw(6) << std::right << std::setprecision(3) << (t_actions2b / t_all) * 100.0
       << "%";
    ss << std::setw(8) << std::right << std::setprecision(3) << "("
       << (t_actions2b / delta_iter) * 1.e3 << " ms/iT)\n";

    ss << std::setw(44) << std::left << "actions3 (IBM):";
    ss << std::setw(6) << std::right << std::setprecision(3) << (t_actions3 / t_all) * 100.0 << "%";
    ss << std::setw(8) << std::right << std::setprecision(3) << "("
       << (t_actions3 / delta_iter) * 1.e3 << " ms/iT)\n";

    ss << std::setw(44) << std::left << "actions4 (Collide and Stream):";
    ss << std::setw(6) << std::right << std::setprecision(3) << (t_actions4 / t_all) * 100.0 << "%";
    ss << std::setw(8) << std::right << std::setprecision(3) << "("
       << (t_actions4 / delta_iter) * 1.e3 << " ms/iT)\n";

    ss << std::setw(44) << std::left << "actions5 (LocMeshToPartVel):";
    ss << std::setw(6) << std::right << std::setprecision(3) << (t_actions5 / t_all) * 100.0 << "%";
    ss << std::setw(8) << std::right << std::setprecision(3) << "("
       << (t_actions5 / delta_iter) * 1.e3 << " ms/iT)\n";

    ss << std::setw(44) << std::left << "actions6 (AdvanceParts):";
    ss << std::setw(6) << std::right << std::setprecision(3) << (t_actions6 / t_all) * 100.0 << "%";
    ss << std::setw(8) << std::right << std::setprecision(3) << "("
    ...
    ss << std::setw(6) << std::right << std::setprecision(3) << (t_logging / t_all) * 100.0 << "%";
    ss << std::setw(8) << std::right << std::setprecision(3) << "("
       << (t_logging / delta_iter) * 1.e3 << " ms/iT)\n";

    ss << std::setw(44) << std::left << "Output:";
    ss << std::setw(6) << std::right << std::setprecision(3) << (t_output / t_all) * 100.0 << "%";
    ss << std::setw(8) << std::right << std::setprecision(3) << "("
       << (t_output / delta_iter) * 1.e3 << " ms/iT)\n";

    ss << std::setw(44) << std::left << "SUM:";
    ss << std::setw(6) << std::right << std::setprecision(3)
       << ((t_reset_com + t_mpi + t_shapeop + t_apply_cont + t_actions1 + t_actions2a + t_actions2b
            + t_actions3 + t_actions4 + t_actions5 + t_actions6 + t_actionsForcingTerm + t_output
            + t_logging)
           / t_all)
              * 100.0
       << "%";
...
void saveAllBlocks(Group3D &group, plint iteration)
{
    ...
    saveState(
        checkpointBlocks, iteration, saveDynamicContent, continueFile, checkpointFile,
        sp.fileNamePadding);
}
...
void copy_data_to_gpu(Solver_GPU &sGPU, vector<Solver *> &cpu_solvers)
{
    ...
    for (ShapeOp_Solver *s : cpu_solvers) {
        for (int i = 0; i < s->verticesOfCollidingPieces_.size(); i++) {
            neighbor_vertices.block(0, k, 3, s->verticesOfCollidingPieces_[i].cols()) =
                s->verticesOfCollidingPieces_[i];
            neighbor_normals.block(0, k, 3, s->normalsOfCollidingPieces_[i].cols()) =
                s->normalsOfCollidingPieces_[i];
            k += s->verticesOfCollidingPieces_[i].cols();
        }
    }
    sGPU.add_collinding_points(
        neighbor_vertices.data(), neighbor_normals.data(), neighbor.data(), total_neighbor);
}
...
int main(int argc, char *argv[])
{
    ...
    if (argc > 2)
        sp.number_nodes = atoi(argv[2]);
    if (argc > 3)
        sp.number_devices =
            atoi(argv[3]);  // number of devices per node
    ...
    xmlFile["ShapeOp_General"]["max_attempts_to_solve_stagnation"].read(
    ...
    xmlFile["ShapeOp_RBC"]["applyGlobalVolumeConservation"].read(
        sp.applyGlobalVolumeConservation_ShapeOp_RBC);
    xmlFile["ShapeOp_RBC"]["globalVolumeConservationWeight"].read(
        sp.globalVolumeConservationWeight_ShapeOp_RBC);
    // ShapeOp: PLTs
    ...
    xmlFile["ShapeOp_PLT"]["applyGlobalVolumeConservation"].read(
        sp.applyGlobalVolumeConservation_ShapeOp_PLT);
    xmlFile["ShapeOp_PLT"]["globalVolumeConservationWeight"].read(
        sp.globalVolumeConservationWeight_ShapeOp_PLT);
    ...
    // These values (miu,lambda,kappa) are the parameters of the SurfaceMaterial Constraint for the
    // npFEM solver: see Case_Studies/RBC(PLT)_X/constraints.csv For the GPU-version they are
    // manually set from here. TODO: Change it as in CPU-version within shapeOpWrapper.h(cpp)
    params_rbc.miu = 35.0;
    ...
    setPointsFromCSV(
        sp.RBC_shapeOpSolverTemplate,
        "./Case_Studies/" + sp.Case_Study_RBC + "/"
            + "points.csv");  // Read all points (internal + OnSurface)
    setConstraintsFromCSV(
        sp.RBC_shapeOpSolverTemplate,
        "./Case_Studies/" + sp.Case_Study_RBC + "/" + "constraints.csv");
    setConnectivityListFromCSV(
        sp.RBC_shapeOpSolverTemplate,
        "./Case_Studies/" + sp.Case_Study_RBC + "/"
            + "surface_connectivity.csv");  // Surface Connectivity (tetrahedra are not included)
    setOnSurfaceParticle(
        sp.RBC_shapeOpSolverTemplate,
        "./Case_Studies/" + sp.Case_Study_RBC + "/" + "onSurfaceParticle.csv");
    Matrix3X RBC_forces(3, sp.RBC_shapeOpSolverTemplate.getPoints().cols());
    RBC_forces.setZero();
    addVertexForce(sp.RBC_shapeOpSolverTemplate, RBC_forces);
    sp.RBC_shapeOpSolverTemplate.initialize(
        sp.Calpha_ShapeOp_RBC, sp.Cbeta_ShapeOp_RBC, sp.dt_p * (T)(sp.timestep_ShapeOp),
        sp.rho_ShapeOp_RBC, false, sp.applyGlobalVolumeConservation_ShapeOp_RBC,
        sp.globalVolumeConservationWeight_ShapeOp_RBC, sp.dx_p, sp.dt_p);

    // Create the PLT ShapeOp template.
    sp.PLT_shapeOpSolverTemplate.bodyType_ = 1;
    setPointsFromCSV(
        sp.PLT_shapeOpSolverTemplate,
        "./Case_Studies/" + sp.Case_Study_PLT + "/"
            + "points.csv");  // Read all points (internal + OnSurface)
    setConstraintsFromCSV(
        sp.PLT_shapeOpSolverTemplate,
        "./Case_Studies/" + sp.Case_Study_PLT + "/" + "constraints.csv");
    setConnectivityListFromCSV(
        sp.PLT_shapeOpSolverTemplate,
        "./Case_Studies/" + sp.Case_Study_PLT + "/"
            + "surface_connectivity.csv");  // Surface Connectivity (tetrahedra are not included)
    setOnSurfaceParticle(
        sp.PLT_shapeOpSolverTemplate,
        "./Case_Studies/" + sp.Case_Study_PLT + "/" + "onSurfaceParticle.csv");
    Matrix3X PLT_forces(3, sp.PLT_shapeOpSolverTemplate.getPoints().cols());
    PLT_forces.setZero();
    addVertexForce(sp.PLT_shapeOpSolverTemplate, PLT_forces);
    sp.PLT_shapeOpSolverTemplate.initialize(
        sp.Calpha_ShapeOp_PLT, sp.Cbeta_ShapeOp_PLT, sp.dt_p * (T)(sp.timestep_ShapeOp),
        sp.rho_ShapeOp_PLT, false, sp.applyGlobalVolumeConservation_ShapeOp_PLT,
        sp.globalVolumeConservationWeight_ShapeOp_PLT, sp.dx_p, sp.dt_p);
    ...
    if (sp.withBodies) {
        ...
        } else  // CellPacking
        {
                ...
                || sp.Simulation_Type.compare("Box_Poiseuille_Flow") == 0)
            {
                ...
                Vtot =
                    sp.PI_ * (sp.pipeRadius_LB * sp.dx_p) * (sp.pipeRadius_LB * sp.dx_p) * sp.lz_p;
            }
            ...
            if (!sp.restartFromCP) {
                    ...
                    while (i < (sp.numRBC + sp.numPLT)) {
                        if (sp.Simulation_Type.compare("Poiseuille_Flow") == 0) {
                            do {
                                ...
                                radial_pos = sqrt(
                                    pow(position[0] - pipeCenter[0], 2)
                                    + pow(position[1] - pipeCenter[1], 2));
                ...
                MPI_Bcast(
                    &sp.iniPositions[0][0], (int)((sp.numRBC + sp.numPLT) * 3), MPI_DOUBLE, 0,
                    MPI_COMM_WORLD);
                MPI_Bcast(
                    &sp.iniRotations[0][0], (int)((sp.numRBC + sp.numPLT) * 3), MPI_DOUBLE, 0,
                    MPI_COMM_WORLD);
            } else {
                pcout << "There is no option to restart from CP in the CellPacking ... ABORT!"
                      << std::endl;
                exit(EXIT_FAILURE);
            }
        }
    ...
    for (pluint jmp = 0; jmp <= max_iTs; jmp += sp.checkpoint_timestep) {
        if (file_exist(
                createFileName(sp.CP_OutputDir + "vels_body_ID_0_iT_", jmp, sp.fileNamePadding)
                + ".csv"))
        {
            sp.continuePLB = true;
            break;
    ...
    iniShapeOpSolvers(
        "points.csv", "constraints.csv", "surface_connectivity.csv", "onSurfaceParticle.csv");

    ///////////////////////////////////////////////////////////////////////////

#ifdef NPFEM_CUDA
    // GPU init
    auto init_gpu = [&](Solver_GPU &sGPU, Solver &s_template, std::vector<Solver *> &solvers,
                        plb::Array<double, 3> *iniPositions, vector<int> *mesh_graph,
                        Mesh_info params) {
        sGPU = Solver_GPU(
            s_template.getPoints(), s_template.getConnectivityList(),
            s_template.get_onSurfaceParticle(), s_template.getConstraints(), params,
            s_template.Cbeta_, solvers.size(), sp.dt_p, (T)(sp.timestep_ShapeOp),
            solvers[0]->bodyID_, mesh_graph);
    ...
    if (sp.loc_rank < sp.number_devices) {
        ...
        if (sp.shapeOpRBCs.size() > 0)
            init_gpu(
                sp.sGPU, sp.RBC_shapeOpSolverTemplate, sp.shapeOpRBCs, sp.iniPositions.data(),
                sp.shapeOpBodies[getSolverID(sp.shapeOpRBCs[0]->bodyID_, sp.shapeOpBodies)]
                    .mesh_graph,
                params_rbc);

        if (sp.shapeOpPLTs.size() > 0)
            init_gpu(
                sp.sGPU_plt, sp.PLT_shapeOpSolverTemplate, sp.shapeOpPLTs, sp.iniPositions.data(),
                sp.shapeOpBodies[getSolverID(sp.shapeOpPLTs[0]->bodyID_, sp.shapeOpBodies)]
                    .mesh_graph,
                params_plt);
    }
#endif
    ...
    if (sp.Simulation_Type.compare("Shear_Flow") == 0) {
        group.add(
            new MultiBlockLattice3D<T, DESCRIPTOR>(
                nx, ny, nz, new CompleteRegularizedBGKdynamics<T, DESCRIPTOR>(omega)),
            "lattice");
        ...
        group.add(
            new MultiBlockLattice3D<T, DESCRIPTOR>(nx, ny, nz, bbDynamics->clone()), "lattice");

        group.getLattice<T, DESCRIPTOR>("lattice").periodicity().toggle(0, true);
        group.getLattice<T, DESCRIPTOR>("lattice").periodicity().toggle(
            1, false);  // y-direction Walls
        group.getLattice<T, DESCRIPTOR>("lattice").periodicity().toggle(2, true);
        defineDynamics(
            group.getLattice<T, DESCRIPTOR>("lattice"), group.getBoundingBox(),
            bbDynamics->clone());
...
#ifdef FORCED
        Dynamics<T, DESCRIPTOR> *dynamics = new GuoExternalForceBGKdynamics<T, DESCRIPTOR>(omega);
        ...
        sp.incompressibleModel = false;
#endif
        defineDynamics(group.getLattice<T, DESCRIPTOR>("lattice"), initDomain, dynamics->clone());
        ...
        group.getLattice<T, DESCRIPTOR>("lattice").periodicity().toggle(
            1, false);  // y-direction Walls
        group.getLattice<T, DESCRIPTOR>("lattice").periodicity().toggle(2, true);  // z-direction
        defineDynamics(
            group.getLattice<T, DESCRIPTOR>("lattice"), group.getBoundingBox(),
            bbDynamics->clone());
        ...
        defineDynamics(
            group.getLattice<T, DESCRIPTOR>("lattice"), initDomain,
            new Pipe<T>(pipeCenter_LB, sp.pipeRadius_LB), bbDynamics->clone());
        ...
        setExternalVector(
            ...
            group.getLattice<T, DESCRIPTOR>("lattice").getBoundingBox(),
    ...
    plb::global::logfile("progress.log")
        .flushEntry(getMultiBlockInfo(group.getLattice<T, DESCRIPTOR>("lattice")));
    ...
    if (sp.continuePLB) {
        ...
        entry["name"].setString(
            FileName(fname_base).defaultPath(global::directories().getOutputDir()));
    ...
    if (sp.continuePLB) {
        do {
            ...
            sp.OutputDir + util::val2str(CP_ID) + "_body_" + util::val2str(sp.numRBC)
            + "_ComPos.log"));
    }
    ...
    plb::global::logfile("progress.log")
        .flushEntry("Creating Actions 1 : BoxRhoBarJPiNeqfunctional3D");
    Actions3D actions1;
    ...
    plint idRhoBarJ_1 = actions1.addProcessor(
        new BoxRhoBarJPiNeqfunctional3D<T, DESCRIPTOR>(), latticeID_act1, rhoBarID_act1, jID_act1,
        piNeqID_act1, group.getBoundingBox());
        ...
        || sp.Simulation_Type.compare("Box_Poiseuille_Flow") == 0)
    {
        ...
        actionsForcingTerm.addProcessor(
            new AddConstForceToMomentum3D<T, DESCRIPTOR>(drivingForce_lb_vec), latticeID_actFT,
            rhoBarID_actFT, jID_actFT, group.getBoundingBox());
    ...
    actions2a.addProcessor(
        new ConstructLocalMeshesFromParticles<T, DESCRIPTOR>(
            sp.rbcTemplate, sp.pltTemplate, sp.bodyToType, sp.particleEnvelopeWidth),
    ...
    T densityOffset = 1.0;
    actions2b.addProcessor(
        new CollisionsForcesCombo<T, DESCRIPTOR>(
            sp.dx_p, omega, densityOffset, Cf, Cp, Ca, sp.collisions_threshold_rep,
            sp.collisions_threshold_nonRep, sp.wallVertexNormals, sp.bodyToType, sp.CellPacking),
    ...
    for (pluint i = 0; i < sp.inamuro_iT; i++) {
        actions3.addProcessor(
            new MultiDirectForcingImmersedBoundaryIteration3D<T, DESCRIPTOR>(
                1. / omega, sp.incompressibleModel, sp.wallMesh),
            particleID_act3, rhoBarID_act3, jID_act3, group.getBoundingBox());
    ...
    actions4.addProcessor(
        new ExternalRhoJcollideAndStream3D<T, DESCRIPTOR>(), latticeID_act4, rhoBarID_act4,
        jID_act4, group.getBoundingBox());
    plint level = 0;
    actions4.addInternalProcessors(
        latticeID_act4, level);  // Add the boundary conditions that were added to the lattices.
    actions4.addCommunication(latticeID_act4, modif::staticVariables);

    // 5. Give new velocities to the particles
    plb::global::logfile("progress.log")
        .flushEntry("Creating Actions 5 : LocalMeshToParticleVelocity3D");
    Actions3D actions5;
    plint particleID_act5 = actions5.addBlock(group.get("vertexParticles"));
    actions5.addProcessor(
        new LocalMeshToParticleVelocity3D<T, DESCRIPTOR>(), particleID_act5,
        group.getBoundingBox());
    actions5.addCommunication(particleID_act5, modif::dynamicVariables);

    // 6. Advance the particles
    plb::global::logfile("progress.log")
        .flushEntry("Creating Actions 6 : AdvanceParticlesEveryWhereFunctional3D");
    Actions3D actions6;
    plint particleID_act6 = actions6.addBlock(group.get("vertexParticles"));
    T cutoffSpeedSqr = -1.0;
    actions6.addProcessor(
        new AdvanceParticlesEveryWhereFunctional3D<T, DESCRIPTOR>(cutoffSpeedSqr), particleID_act6,
        group.getBoundingBox());
    ...
    sp.centers_log =
        MatrixXX(sp.output_timestep / sp.timestep_ShapeOp, 3 * sp.shapeOpBodies.size());
    ...
    // 4. Change the momentum of the fluid using IBM & any other external force (Shan-Chen forcing
    // scheme).
    //    The fluid needs to know the position & velocity of the immersed body at time t and based
    //    on this we are going to advance to t+1. Our schemes here are explicit and so we compute
    //    t+1 with the info that we have at t. This is why we did not advance at the previous step
    ...
    while (true) {
        ...
        plb::global::logfile_nonparallel("particles.log")
            .flushEntry("\n\nStarting Time Loop " + util::val2str(iT));
#endif
        ...
        // Solved but not yet fully advanced at t+1 (SO side at t+1 but not Palabos side which is
        // still at t)
        if (sp.iT % sp.timestep_ShapeOp == 0) {
            ...
            timer("logging").start();
            plb::global::logfile("progress.log")
                .flushEntry("Send forces and Collision Containers (Palabos -> ShapeOp)");
            timer("logging").stop();
            timer("mpi").start();
            sendBodyForcesAndCollisionData<T>(
                sp.bodyToProc,
                sp.shapeOpBodies);  // Send forces from Palabos local meshes to ShapeOp
            timer("mpi").stop();

            timer("logging").start();
            plb::global::logfile("progress.log")
                .flushEntry("Receive forces and Collision Containers (Palabos -> ShapeOp)");
            timer("logging").stop();
            timer("mpi").start();
            receiveBodyForcesAndCollisionData<T>(
                sp.shapeOpBodies);  // Receive forces on the ShapeOp end.
            timer("mpi").stop();
            ...
            if (sp.shapeOpRBCs.size()) {
                ...
                sp.sGPU.solve(
                    sp.max_iterations_ShapeOp, sp.tol_ShapeOp, true, sp.gamma_ShapeOp,
                    sp.max_line_search_loops_ShapeOp, sp.m_ShapeOp, sp.gamma2_ShapeOp);
                // GPU counterpart of imposePlbPeriodicityToSO
                sp.sGPU.make_periodic(
            ...
            for (auto &so : sp.shapeOpRBCs) {
                ...
                so->solve(
                    sp.m_ShapeOp, sp.max_iterations_ShapeOp, sp.max_line_search_loops_ShapeOp,
                    sp.max_attempts_to_solve_stagnation_ShapeOp, sp.convergence_window_ShapeOp,
                    sp.tol_ShapeOp, sp.gamma_ShapeOp, sp.gamma2_ShapeOp,
                    sp.collisions_threshold_rep * sp.dx_p, sp.collisions_weight_rep,
                    sp.collisions_threshold_nonRep * sp.dx_p, sp.collisions_weight_nonRep,
                    sp.beta_morse, vel_cap, vel_cap_fin);
        ...
        if (sp.iT % sp.timestep_ShapeOp == 0) {
            timer("shapeop").start();
            for (pluint i = 0; i < sp.shapeOpBodies.size(); ++i) {
                sp.shapeOpBodies[i].imposePlbPeriodicityToSO(
                    sp.dx_p, group.getLattice<T, DESCRIPTOR>("lattice").getNx(),
        ...
        if (sp.iT % sp.output_timestep == 0 && discardFlag) {
            ...
            if (sp.shapeOpBodies.size()) {
                ...
                if (sp.iT > 0) {
                    ...
                    for (pluint j = 0; j < (pluint)sp.centers_log.cols(); j += 3) {
                        ...
                        if (print) {
                            std::ofstream ofile;
                            ofile.open(
                                sp.OutputDir + util::val2str(CP_ID) + "_body_"
                                    + plb::util::val2str(bodyID_tmp) + "_ComPos.log",
                                std::ofstream::out | std::ofstream::app);
                            for (int i = 0; i < sp.centers_log.rows(); i++) {
                                ofile << plb::util::val2str(
                                             sp.iT - sp.output_timestep + i * sp.timestep_ShapeOp
                                             + sp.timestep_ShapeOp)
                ...
                if (printVTKs) {
                    ...
                    RawConnectedTriangleMesh<T> RBCs_mesh = sp.shapeOpBodies[0].mergeMultipleMeshes(
                        sp.shapeOpRBCs, sp.shapeOpPLTs_empty, sp.dx_p, sp.dump_RBCs_ratio_VTKs,
                        sp.dump_PLTs_ratio_VTKs);
                    RawConnectedTriangleMesh<T> PLTs_mesh = sp.shapeOpBodies[0].mergeMultipleMeshes(
                        sp.shapeOpRBCs_empty, sp.shapeOpPLTs, sp.dx_p, sp.dump_RBCs_ratio_VTKs,
                        sp.dump_PLTs_ratio_VTKs);
                    ...
                    multiProcWriteVTK(
                        RBCs_mesh,
                        createFileName(
                            sp.OutputDir + util::val2str(global::mpi().getRank()) + "_RBCs_iT_",
                            sp.iT, sp.fileNamePadding)
                            + ".vtk",
                        group.getLattice<T, DESCRIPTOR>("lattice").getNx(),
                        group.getLattice<T, DESCRIPTOR>("lattice").getNy(),
                        group.getLattice<T, DESCRIPTOR>("lattice").getNz(), sp.dx_p, sp.dt_p,
                        sp.rho_p);
                    // PLTs
                    multiProcWriteVTK(
                        PLTs_mesh,
                        createFileName(
                            sp.OutputDir + util::val2str(global::mpi().getRank()) + "_PLTs_iT_",
                            sp.iT, sp.fileNamePadding)
                            + ".vtk",
                        group.getLattice<T, DESCRIPTOR>("lattice").getNx(),
                        group.getLattice<T, DESCRIPTOR>("lattice").getNy(),
        ...
        // Access averages from internal statistics ( their value is defined only after the call to
        ...
        if (sp.iT % sp.output_timestep == 0) {
            ...
            pcout << "; av energy=" << setprecision(10)
                  << computeAverageEnergy<T>(group.getLattice<T, DESCRIPTOR>("lattice"))
                  << "; av rho=" << setprecision(10)
                  << computeAverageDensity<T>(group.getLattice<T, DESCRIPTOR>("lattice"))
                  << "; max uLB=" << setprecision(10)
                  << computeMax(*computeVelocityNorm(group.getLattice<T, DESCRIPTOR>("lattice")))
            ...
            timer("logging").start();
            plb::global::logfile("energy.log")
                .flushEntry(util::val2str(
                    computeAverageEnergy<T>(group.getLattice<T, DESCRIPTOR>("lattice"))));
            timer("logging").stop();
        ...
        if (sp.iT % sp.checkpoint_timestep == 0 && sp.iT > 0 && discardFlag) {
            ...
            if (!sp.CellPacking) {
                for (pluint i = 0; i < sp.shapeOpBodies.size(); ++i) {
                    savePointsToCSV(
                        sp.shapeOpBodies[i].getSolver(),
                        createFileName(
                            sp.CP_OutputDir + "body_ID_"
                                + util::val2str(sp.shapeOpBodies[i].getID()) + "_iT_",
                            sp.iT, sp.fileNamePadding)
                            + ".csv",
                        sp.iT, sp.timestep_ShapeOp);
                    saveVelsToCSV(
                        sp.shapeOpBodies[i].getSolver(),
                        createFileName(
                            sp.CP_OutputDir + "vels_body_ID_"
                                + util::val2str(sp.shapeOpBodies[i].getID()) + "_iT_",
                            sp.iT, sp.fileNamePadding)
                            + ".csv");
                }
                ...
                for (pluint i = 0; i < sp.shapeOpBodies.size(); ++i) {
                    savePointsToCSV(
                        sp.shapeOpBodies[i].getSolver(),
                        sp.CP_OutputDir + "body_ID_" + util::val2str(sp.shapeOpBodies[i].getID())
                            + ".csv",
                        sp.iT, sp.timestep_ShapeOp);
                }
            }
            ...
            if (CPs > 2 && !sp.CellPacking) {
                ...
                for (auto &bd : sp.shapeOpBodies) {
                    tmpID = bd.getID();
                    remove((createFileName(
                                sp.CP_OutputDir + "body_ID_" + util::val2str(tmpID) + "_iT_", iter,
                                sp.fileNamePadding)
                            + ".csv")
                               .c_str());
                    remove((createFileName(
                                sp.CP_OutputDir + "vels_body_ID_" + util::val2str(tmpID) + "_iT_",
                                iter, sp.fileNamePadding)
                            + ".csv")
                               .c_str());
                }
                ...
                if (global::mpi().getRank() == 0) {
                    for (pluint i = 0; i < sp.numOfBlocks; ++i) {
                        remove(
                            (createFileName(sp.OutputDir + "checkpoint_", iter, sp.fileNamePadding)
                             + "_" + util::val2str(i) + ".dat")
                                .c_str());
                        remove(
                            (createFileName(sp.OutputDir + "checkpoint_", iter, sp.fileNamePadding)
                             + "_" + util::val2str(i) + ".plb")
                                .c_str());
                    }
                }
            }
