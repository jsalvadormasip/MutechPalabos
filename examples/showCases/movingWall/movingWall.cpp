...
struct Param {
    ...
    plint maxIter, statIter,
        outIter;  // Maximum number of iterations, and number of iterations for output.
    plint startIter;
...
void printParam(void)
{
    ...
    pcout << "mountPoint = (" << param.mountPoint[0] << ", " << param.mountPoint[1] << ", "
          << param.mountPoint[2] << ")" << std::endl;
    ...
    pcout << "mountPointLB = (" << param.mountPointLB[0] << ", " << param.mountPointLB[1] << ", "
          << param.mountPointLB[2] << ")" << std::endl;
...
int main(int argc, char *argv[])
{
    ...
    MultiBlockLattice3D<T, DESCRIPTOR> *lattice =
        new MultiBlockLattice3D<T, DESCRIPTOR>(param.nx, param.ny, param.nz, dynamics->clone());
    defineDynamics(*lattice, lattice->getBoundingBox(), dynamics->clone());
    delete dynamics;
    lattice->toggleInternalStatistics(false);

    MultiScalarField3D<T> *rhoBar =
        generateMultiScalarField<T>((MultiBlock3D &)*lattice, largeEnvelopeWidth).release();
    rhoBar->toggleInternalStatistics(false);

    MultiTensorField3D<T, 3> *j =
        generateMultiTensorField<T, 3>((MultiBlock3D &)*lattice, largeEnvelopeWidth).release();
    ...
    integrateProcessingFunctional(
        new ExternalRhoJcollideAndStream3D<T, DESCRIPTOR>(), lattice->getBoundingBox(), rhoBarJarg,
        0);
    integrateProcessingFunctional(
        new BoxRhoBarJfunctional3D<T, DESCRIPTOR>(), lattice->getBoundingBox(), rhoBarJarg, 2);
    ...
    if (param.periodicBoundaries) {
        ...
        int bcType       = 1;
        integrateProcessingFunctional(
            new VirtualOutlet<T, DESCRIPTOR>(outsideDensity, globalDomain, bcType), outlet, bcargs,
            1);
    ...
    initializeAtEquilibrium(
        *lattice, lattice->getBoundingBox(), (T)1.0, Array<T, 3>(vx, (T)0, (T)0));
    applyProcessingFunctional(
        new BoxRhoBarJfunctional3D<T, DESCRIPTOR>(), lattice->getBoundingBox(), rhoBarJarg);
    ...
    TriangleSet<T> rectangleTriangleSet = constructRectangle<T>(
        param.xSideLB, param.ySideLB, util::roundToInt(2.0 * param.xSideLB),
        util::roundToInt(2.0 * param.ySideLB));
    ...
    for (plint iT = 0; iT < param.maxIter; iT++) {
        ...
        if ((iT % param.statIter == 0 && iT != 0) || iT == param.maxIter - 1) {
            ...
            T energy =
                computeAverageEnergy(*lattice) * (param.dx * param.dx) / (param.dt * param.dt);
        ...
        for (int i = 0; i < param.ibIter; i++) {
            inamuroIteration(
                SurfaceVelocity(timeLB), *rhoBar, *j, container, (T)1.0 / param.omega,
                incompressibleModel);
        }
    }
