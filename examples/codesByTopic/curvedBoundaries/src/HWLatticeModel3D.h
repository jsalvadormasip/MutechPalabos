...
class HWLatticeModel3D : public OffLatticeModel3D<T, Array<T, 3> > {
public:
    HWLatticeModel3D(BoundaryShape3D<T, Array<T, 3> > *shape_, int flowType_);
    virtual HWLatticeModel3D<T, Descriptor> *clone() const;
...
private:
    ...
    ///   populations.
    class HWLatticeInfo3D : public ContainerBlockData {
    public:
        HWLatticeInfo3D() : localForce(Array<T, 3>::zero()) { }
        ...
        virtual HWLatticeInfo3D *clone() const
        {
            return new HWLatticeInfo3D(*this);
        }
