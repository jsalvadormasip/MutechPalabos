...
public:
    ...
    double magic {};
    void parse(
        symbol_table_t &table, expression_t &expression, std::string &expression_str,
        parser_t &parser)
    {
        ...
        if (not parser.compile(expression_str, expression)) {
            pcout << "ERROR PARSING EXPRESSION " << expression_str << ": Compilation error..."
                  << std::endl;
...
enum class BCmodel { HW, BFL, FH, MLS, ELIULC, LIgeneric, ELIUL, ELIULK1, ELIULK3, ELIULK4 };
/**
...
auto voxelize_helper(const IncomprFlowParam<Real> parameters, TriangleSet<Real> *triangle_set)
{
    ...
    plint blockSize = 0;    // Size of blocks in the sparse/parallel representation. Zero means:
                            // don't use sparse representation.
    auto defMesh = new DEFscaledMesh<Real>(*triangle_set, 0, xDirection, margin, Dot3D(0, 0, 0));
    ...
    auto bounding_box =
        Box3D(0, parameters.getNx() - 1, 0, parameters.getNy() - 1, 0, parameters.getNz() - 1);
    auto voxalized_domain = new VoxelizedDomain3D<Real>(
        *boundary, voxelFlag::outside, bounding_box, borderWidth, extendedEnvelopeWidth, blockSize);
    pcout << getMultiBlockInfo(voxalized_domain->getVoxelMatrix()) << std::endl;

    return std::tuple {voxalized_domain, boundary};
}

enum class ProcessorLevel { offLattice = 1, rhoBarJ = 2 };
/**
...
auto inject_off_lattice_bc(
    ...
    VoxelizedDomain3D<Real> *voxalized_domain, BCmodel kmin_, BSchemeInfo &xmlParse)
{
    ...
    defineDynamics(
        *target_lattice, voxalized_domain->getVoxelMatrix(), target_lattice->getBoundingBox(),
    ...
    auto coefficients = [&xmlParse](Real q, Real tauPlus, Real tauMinus) -> std::array<Real, 5> {
        xmlParse.q = q;
    ...
    boundaryCondition = new OffLatticeBoundaryCondition3D<Real, Descriptor, Array3D>(
    ...
    integrateProcessingFunctional(
        new PackedRhoBarJfunctional3D<Real, Descriptor>(), target_lattice->getBoundingBox(),
        *target_lattice, *rhoBarJfield, (plint)ProcessorLevel::rhoBarJ);

    //    initializeAtEquilibrium(
    //            *target_lattice, target_lattice->getBoundingBox(), 1., Array<Real, 3>(0., 0.,
    //            0.));
    //    target_lattice->initialize();
    applyProcessingFunctional(
        new PackedRhoBarJfunctional3D<Real, Descriptor>(), target_lattice->getBoundingBox(),
        *target_lattice, *rhoBarJfield);
 ...
 */
template <typename Real, template <typename U> class Descriptor>
auto inject_on_lattice_bc(
    MultiBlockLattice3D<Real, Descriptor> *target_lattice, IncomprFlowParam<Real> const &parameters,
    Array<Real, 3> spherePosition)
{
    ...
    setBoundaryVelocity(
        *target_lattice, target_lattice->getBoundingBox(), ConstantVelocity<Real>(parameters));
    setBoundaryDensity(*target_lattice, outlet, ConstantDensity<Real>(1.0));
    initializeAtEquilibrium(
...
void createSpongeZones(
    IncomprFlowParam<Real> const &parameters, Array<plint, 6> const &numSpongeCells,
    MultiBlockLattice3D<Real, Descriptor> *lattice, bool useSmagorinskySponges = false,
    Real bulkValue = 0.01, Real targetValue = 0.5)
{
    if (std::accumulate(&numSpongeCells[0], &numSpongeCells[5], (Real)0.0) > 0) {
        if (useSmagorinskySponges) {
            ...
            applyProcessingFunctional(
                new SmagorinskySpongeZone3D<Real, Descriptor>(
                    parameters.getNx(), parameters.getNy(), parameters.getNz(), bulkValue,
                    targetValue, numSpongeCells),
