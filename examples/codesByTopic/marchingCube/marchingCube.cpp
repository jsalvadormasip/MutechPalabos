...
class ShapeFunction {
public:
    ShapeFunction(Array<plint, 3> const &c1_, Array<plint, 3> const &c2_) : c1(c1_), c2(c2_) { }
    T operator()(plint iX, plint iY, plint iZ)
    {
        return std::min(
            std::sqrt(
                (T)util::sqr(iX - c1[0]) + (T)util::sqr(iY - c1[1]) + (T)util::sqr(iZ - c1[2])),
            std::sqrt(
                (T)util::sqr(iX - c2[0]) + (T)util::sqr(iY - c2[1]) + (T)util::sqr(iZ - c2[2])));
    }
...
public:
    SphereFunction(Array<plint, 3> const &center_, plint radius_) : center(center_), radius(radius_)
    { }
    ...
    {
        return normSqr(Array<T, 3>(pos[0] - center[0], pos[1] - center[1], pos[2] - center[2]))
               < radius * radius;
    }
...
int main(int argc, char *argv[])
{
    ...
    setToFunction(
        scalarField, scalarField.getBoundingBox(),
        ShapeFunction<T>(Array<plint, 3>(40, 40, 40), Array<plint, 3>(140, 140, 140)));

    // Alternative way to get the first application: through an analytical description.
    typedef TriangleSet<T>::Triangle Triangle;
    std::vector<Triangle> triangles;
    isoSurfaceMarchingCube(
        triangles, scalarField, isoLevels, scalarField.getBoundingBox().enlarge(-1));
    TriangleSet<T>(triangles).writeAsciiSTL("iso.stl");

    std::vector<Triangle> triangles2;
    isoSurfaceMarchingCube<T, SphereFunction>(
        triangles2, scalarField, SphereFunction(Array<plint, 3>(40, 40, 40), 50),
    ...
    isoSurfaceMarchingCube(
        triangles, voxelizedDomain, voxelizedDomain.getVoxelMatrix().getBoundingBox());
