    ...
    for (plint iX = 0; iX < (plint)xVal.size() - 1; ++iX) {
        for (plint iY = 0; iY < (plint)yVal.size() - 1; ++iY) {
            for (plint iZ = 0; iZ < (plint)zVal.size() - 1; ++iZ) {
                ...
                pcout << "Adding block with ID=" << nextID << ": [" << domain.x0 << "," << domain.x1
                      << " | " << domain.y0 << "," << domain.y1 << " | " << domain.z0 << ","
                      << domain.z1 << "]" << std::endl;
            }
        }
    }
...
int main(int argc, char *argv[])
{
    ...
    SparseBlockStructure3D blockStructure(
        createCavityDistribution3D(parameters.getNx(), parameters.getNy(), parameters.getNz()));
    ...
    // @Tomasz: STEP 2
    // Here the co-processor is informed about the domains for which it will need to do
    // computations.
    bool printInfo = true;
    initiateCoProcessors(
        lattice, BGKdynamics<T, DESCRIPTOR>(parameters.getOmega()).getId(), printInfo);
    ...
    for (plint iT = 0; iT < parameters.nStep(maxT); ++iT) {
        ...
        if (iT % parameters.nStep(logT) == 0) {
            pcout << "; av energy=" << setprecision(10) << computeAverageEnergy<T>(lattice)
                  << "; av rho=" << setprecision(10) << computeAverageDensity<T>(lattice) << endl;
