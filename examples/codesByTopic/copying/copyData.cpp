...
void copyBlockLattice3D()
{
    ...
    defineDynamics(lattice3d_1, domain1_3d, new BounceBack<double, D3Q19Descriptor>(1.));
    ...
    std::unique_ptr<MultiBlockLattice3D<double, D3Q19Descriptor> > lattice3d_3 =
        clone(lattice3d_1, domain1_3d);
}
...
void copyBlockLattice2D()
{
    ...
    defineDynamics(lattice2d_1, domain1_2d, new BounceBack<double, D2Q9Descriptor>(1.));
    // Copy everything, and recreate a dynamics object at the destination.
    ...
    // 3. CREATING CLONES.
    // Create a reduced-size clone, correponding to a sub-domain of the original lattice.
    std::unique_ptr<MultiBlockLattice2D<double, D2Q9Descriptor> > lattice2d_3 =
        clone(lattice2d_1, domain1_2d);
}
...
void copyScalarField3D()
{
    ...
    // 3. CREATING CLONES.
    std::unique_ptr<MultiScalarField3D<double> > scalar3d_4 = clone(scalar3d_2, domain1_3d);
}
...
void copyScalarField2D()
{
    ...
    // 3. CREATING CLONES.
    std::unique_ptr<MultiScalarField2D<int> > scalar2d_4 = clone(scalar2d_3, domain1_2d);
}
