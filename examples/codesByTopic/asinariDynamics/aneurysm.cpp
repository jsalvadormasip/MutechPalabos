    ...
    T dt)
{
    ...
    vtkOut.writeData<float>(
        *boundaryCondition.computePressure(vtkDomain), "p", util::sqr(dx / dt) * fluidDensity);
    vtkOut.writeData<float>(*boundaryCondition.computeVelocityNorm(vtkDomain), "u", dx / dt);
    vtkOut.writeData<float>(
        *copyConvert<int, T>(
            *extractSubDomain(boundaryCondition.getVoxelizedDomain().getVoxelMatrix(), vtkDomain)),
        "voxel", 1.);
...
void writeImages(
    OffLatticeBoundaryCondition3D<T, DESCRIPTOR, Velocity> &boundaryCondition, plint level,
    Array<T, 3> location, T dx, T dt)
{
    ...
    writeImages(
        boundaryCondition, xy_imageDomain, xy_vtkDomain, "xy_" + util::val2str(level), location, dx,
        dt);
    writeImages(
        boundaryCondition, xz_imageDomain, xz_vtkDomain, "xz_" + util::val2str(level), location, dx,
        dt);
    writeImages(
        boundaryCondition, yz_imageDomain, yz_vtkDomain, "yz_" + util::val2str(level), location, dx,
        dt);
}
    ...
    plint level, MultiBlockLattice3D<T, DESCRIPTOR> *iniVal = 0)
{
    ...
    if (useIncompressible) {
        dynamics = new IncAsinariDynamics<T, DESCRIPTOR>(
            omega);  // At this model velocity equals momentum.
    } else {
        dynamics =
            new AsinariDynamics<T, DESCRIPTOR>(omega);  // At this model velocity equals momentum
                                                        //   divided by density.
    }
#else
    if (useIncompressible) {
        dynamics =
            new IncBGKdynamics<T, DESCRIPTOR>(omega);  // At this model velocity equals momentum.
    } else {
    ...
    integrateProcessingFunctional(
        new AsinariPostCollide3D<T, DESCRIPTOR>, lattice->getBoundingBox(), *lattice, 0);
#endif
    ...
    GuoOffLatticeModel3D<T, DESCRIPTOR> *model = new GuoOffLatticeModel3D<T, DESCRIPTOR>(
        ...
        useAllDirections);
    model->setVelIsJ(
        useIncompressible);  // When the incompressible BGK model is used, velocity equals momentum.
    ...
    T inletPressure =
        DESCRIPTOR<T>::cs2 * (boundaryCondition.computeAverageDensity(measureBox) - 1.);
