...
void boundarySetup( MultiBlockLattice2D<T,DESCRIPTOR>& lattice,
...
void cylinderSetup( IncomprFlowParam<T> const& parameters,
                    ...
                    T dV )
{
    ...
    for (int i = 0; i < nodeNum; ++i) {
        ...
        vertices.push_back(Array<T, 2>(cx + radius * std::cos(angle), cy + radius * std::sin(angle)));
        areas.push_back(dV);
    }
}
...
int main(int argc, char* argv[]) {
    ...
    MultiScalarField2D<T> *rhoBar = generateMultiScalarField<T>((MultiBlock2D&) *lattice, largeEnvelopeWidth).release();
    rhoBar->toggleInternalStatistics(false);
    MultiTensorField2D<T,2> *j = generateMultiTensorField<T,2>((MultiBlock2D&) *lattice, largeEnvelopeWidth).release();
    ...
    integrateProcessingFunctional(
          new BoxRhoBarJfunctional2D<T,DESCRIPTOR>(),
          lattice->getBoundingBox(), rhoBarJarg, 1);
    ...
    for (plint iT=0; iT*parameters.getDeltaT()<maxT; ++iT) {
       ...
       if (iT%parameters.nStep(imSave)==0) {
        ...
        for (int i = 0; i < ibIter; ++i) {
            inamuroIteration(SurfaceVelocity(iT + 1.0),
                    *rhoBar, *j, container, (T) parameters.getTau(), false);
        }
        ...
        if (iT%parameters.nStep(logT)==0) {
            pcout << "; av energy ="
                  ...
                  << "; av rho ="
                  << computeAverageDensity<T>(*lattice) << endl;
        }
    }
