    ...
    OnLatticeBoundaryCondition2D<T, DESCRIPTOR> &boundaryCondition)
{
    ...
    defineDynamics(
        lattice, lattice.getBoundingBox(), new BounceBackNodes<T>(N, radius),
...
int main(int argc, char *argv[])
{
    ...
    MultiBlockManagement2D sparseBlockManagement = computeSparseManagement(
        *plb::reparallelize(flagMatrix, blockSize, blockSize), envelopeWidth);

    // Instantiate the multi-block, based on the created block distribution and
    // on default parameters.
    MultiBlockLattice2D<T, DESCRIPTOR> lattice(
        sparseBlockManagement, defaultMultiBlockPolicy2D().getBlockCommunicator(),
