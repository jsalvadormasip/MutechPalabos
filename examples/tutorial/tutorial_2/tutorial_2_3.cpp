...
public:
    BounceBackNodes(plint N, plint radius) : cx(N / 2), cy(N / 2), innerR(radius), outerR(N / 2) { }
 ...
 */
void createHalfCircleManually(MultiBlockLattice2D<T, DESCRIPTOR> &lattice, plint N, plint radius)
{
...
class HalfCircleInstantiateFunctional : public BoxProcessingFunctional2D_L<T, Descriptor> {
public:
    HalfCircleInstantiateFunctional(plint N_, plint radius_) : N(N_), radius(radius_) { }
        ...
        for (plint iX = domain.x0; iX <= domain.x1; ++iX) {
            for (plint iY = domain.y0; iY <= domain.y1; ++iY) {
                if (bbDomain(iX + relativeOffset.x, iY + relativeOffset.y)) {
                    lattice.attributeDynamics(iX, iY, new BounceBack<T, DESCRIPTOR>);
                }
            }
        }
    }
    ...
    MultiBlockLattice2D<T, DESCRIPTOR> &lattice, plint N, plint radius)
{
    applyProcessingFunctional(
        new HalfCircleInstantiateFunctional<T, DESCRIPTOR>(N, radius), lattice.getBoundingBox(),
        lattice);
}
...
int main(int argc, char *argv[])
{
    ...
    MultiBlockLattice2D<T, DESCRIPTOR> lattice(
        MultiBlockManagement2D(
            sparseBlock, defaultMultiBlockPolicy2D().getThreadAttribution(), envelopeWidth),
