...
// This is the "send" part of the MPI communication for forces (Palabos->ShapeOp) & Collision
// Containers
template <typename T>
void sendBodyForcesAndCollisionData(
    std::map<pluint, pluint> &bodyToProc, std::vector<ShapeOpBody<T>> &shapeOpBodies)
{
    for (typename std::map<pluint, LocalMesh<T> *>::iterator it = LocalMeshes<T>().begin();
         it != LocalMeshes<T>().end(); ++it)
    {
        ...
        plb::global::logfile_nonparallel("localProgress.log")
            .flushEntry(
                "Local Mesh with ID=" + util::val2str(it->first)
                + " | numVertices=" + util::val2str(it->second->numVertices)
                + ", at Processor=" + util::val2str(global::mpi().getRank())
        ...
        if (processorToSend == (pluint)global::mpi().getRank()) {
            ...
            shapeOpBodies[solverID].receiveContainers(
                it->second->vertexIDs, it->second->shearForces, it->second->normals,
...
void receiveBodyForcesAndCollisionData(std::vector<ShapeOpBody<T>> &shapeOpBodies)
{
    ...
    plb::global::logfile_nonparallel("localProgress.log")
        .flushEntry("RECV Forces & Collision Containers");
    if (isComplete)
        plb::global::logfile_nonparallel("localProgress.log")
            .flushEntry("Communication Complete - NO MPI");
    else
        plb::global::logfile_nonparallel("localProgress.log")
            .flushEntry("Communication NOT Complete - MPI");
#endif  // ENABLE_LOGS
    ...
    while (!isComplete) {
...
#ifdef ENABLE_LOGS
        plb::global::logfile_nonparallel("localProgress.log")
            .flushEntry("MPI_Recv - MPI_ANY_SOURCE");
#endif  // ENABLE_LOGS

        MPI_Recv(
            &bodyID, 1, MPI_UNSIGNED_LONG_LONG, MPI_ANY_SOURCE,
        ...
        plb::global::logfile_nonparallel("localProgress.log")
            .flushEntry(
                "MPI_Recv - From: " + util::val2str(sending_processor)
        ...
        MPI_Recv(
            &numVertices, 1, MPI_UNSIGNED_LONG_LONG, sending_processor,
            shapeOpMPItagForcesAndCollisionContainersNumVertices, MPI_COMM_WORLD, &dummyStatus);
#ifdef ENABLE_LOGS
        plb::global::logfile_nonparallel("localProgress.log")
            .flushEntry("numVertices=" + util::val2str(numVertices));
#endif  // ENABLE_LOGS
        ...
        MPI_Recv(
            &vertexIDs[0], (int)numVertices, MPI_UNSIGNED_LONG_LONG, sending_processor,
            shapeOpMPItagForcesAndCollisionContainersVertexIDs, MPI_COMM_WORLD, &dummyStatus);
#ifdef ENABLE_LOGS
        plb::global::logfile_nonparallel("localProgress.log").flushEntry("Received vertexID");
#endif  // ENABLE_LOGS

        MPI_Recv(
            &shearForces[0][0], (int)(3 * numVertices), MPI_DOUBLE, sending_processor,
            shapeOpMPItagForcesAndCollisionContainersShearForces, MPI_COMM_WORLD, &dummyStatus);
#ifdef ENABLE_LOGS
        plb::global::logfile_nonparallel("localProgress.log").flushEntry("Received shearForces");
#endif  // ENABLE_LOGS

        MPI_Recv(
            &normals[0][0], (int)(3 * numVertices), MPI_DOUBLE, sending_processor,
            shapeOpMPItagForcesAndCollisionContainersNormals, MPI_COMM_WORLD, &dummyStatus);
#ifdef ENABLE_LOGS
        plb::global::logfile_nonparallel("localProgress.log").flushEntry("Received normals");
#endif  // ENABLE_LOGS

        MPI_Recv(
            &pressure[0], (int)(numVertices), MPI_DOUBLE, sending_processor,
            shapeOpMPItagForcesAndCollisionContainersPressure, MPI_COMM_WORLD, &dummyStatus);
#ifdef ENABLE_LOGS
        plb::global::logfile_nonparallel("localProgress.log").flushEntry("Received pressure");
#endif  // ENABLE_LOGS

        MPI_Recv(
            &area[0], (int)(numVertices), MPI_DOUBLE, sending_processor,
        ...
        MPI_Recv(
            &numCollidingNeighbors, 1, MPI_UNSIGNED_LONG_LONG, sending_processor,
            shapeOpMPItagForcesAndCollisionContainersNumCollidingNeighbors, MPI_COMM_WORLD,
            &dummyStatus);
#ifdef ENABLE_LOGS
        plb::global::logfile_nonparallel("localProgress.log")
            .flushEntry("Received numCollidingNeighbors=" + util::val2str(numCollidingNeighbors));
#endif  // ENABLE_LOGS
        ...
        if (numCollidingNeighbors > 0) {
            MPI_Recv(
                ...
                sending_processor, shapeOpMPItagForcesAndCollisionContainersCollisionNeighbors,
                MPI_COMM_WORLD, &dummyStatus);
#ifdef ENABLE_LOGS
            ...
            MPI_Recv(
                ...
                sending_processor,
                shapeOpMPItagForcesAndCollisionContainerscollisionNeighborsNormals, MPI_COMM_WORLD,
                &dummyStatus);
#ifdef ENABLE_LOGS
            plb::global::logfile_nonparallel("localProgress.log")
                .flushEntry("Received neighborsNormals");
#endif  // ENABLE_LOGS
        }

        shapeOpBodies[solverID].receiveContainers(
            vertexIDs, shearForces, normals, pressure, area, sending_processor, collisionNeighbors,
            collisionNeighborsNormals);
        ...
        if (isComplete)
            plb::global::logfile_nonparallel("localProgress.log")
                .flushEntry("Communication Complete");
        else
            plb::global::logfile_nonparallel("localProgress.log")
                .flushEntry("Communication NOT Complete - Re-enter MPI Loop");
#endif  // ENABLE_LOGS
    }
    ...
    T rho)
{
    ...
    for (; it != LocalMeshes<T>().end(); ++it) {
        ...
        } else  // Treating MPI Case
        {
            ...
            MPI_Recv(
                &velocities[0][0], (int)(3 * it->second->numVertices), MPI_DOUBLE, sendingProcessor,
                shapeOpMPItagVelocities, MPI_COMM_WORLD, &dummyStatus);
        }
        ...
        for (pluint i = 0; i < (pluint)it->second->vertexIDs.size(); ++i) {
            ...
            it->second->velocities[vertexID] =
                Array<T, 3>(velocities[i][0], velocities[i][1], velocities[i][2]) * Cu;
        }
    }
}
